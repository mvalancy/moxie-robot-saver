...
hold a code up to its camera. Built on the [recovered protobuf schemas](../../docs/reverse-engineering/protocol/recovered-proto/).
...
| `moxie_toolkit/markup.py` | Build `<mark name="cmd:…">` behavior markup (gestures, mood, audio) to weave into TTS. See [behavior-markup.md](../../docs/reverse-engineering/runtime/behavior-markup.md). |
| `secrets/emulate_secrets.py` | Recover factory secrets from `libsecrets.so` via Unicorn emulation (algorithm = XOR with package name). See [factory-provisioning.md](../../docs/reverse-engineering/firmware/factory-provisioning.md). |
| `moxie_toolkit/cloud.py` | MQTT transport helpers: topic builders, the `/commands/zmq` `name:bytes` framing, event/command envelope parse/build, the full **telehealth remote-puppet** session (`telehealth_session()`, `telehealth_play_output()`, `telehealth_command()`, `telehealth_topic()`, `parse_telehealth_event()` — see [telehealth.md](../../docs/reverse-engineering/protocol/telehealth.md)), and **`service_configuration()`** (repoint a robot: host/port/disable_verify), plus the **config/telemetry data-model** helpers `build_robot_cloud_config()`/`parse_robot_cloud_config()` (the `/config` document), `parse_robot_status()`, `parse_telemetry_packet()`, `parse_cloud_status()` (see [device-config-and-telemetry.md](../../docs/reverse-engineering/protocol/device-config-and-telemetry.md)), and the **RemoteChat** brain-reply helpers `remote_chat_reply()`/`remote_chat_action()`/`parse_remote_chat_request()`/`parse_remote_chat_response()` (see [remote-chat-protocol.md](../../docs/reverse-engineering/protocol/remote-chat-protocol.md)), plus the **offline brain-state** parsers `parse_fallback_info()`/`parse_cs_data()`/`parse_user_recommendation_data()` (see [offline-and-brain-state.md](../../docs/reverse-engineering/protocol/offline-and-brain-state.md)). and the **SEL taxonomy** parsers `parse_sel_tag_info()`/`parse_module_tag_info()` (see [content-and-conversation.md](../../docs/reverse-engineering/runtime/content-and-conversation.md)). See [cloud-protocol.md](../../docs/reverse-engineering/protocol/cloud-protocol.md). |
| `moxie_toolkit/bus.py` | **MoxieBus** — the on-device ZMQ bus client (drive face/motors/LEDs, read sensors); `fused_people_classes()` for the [perception-fusion](../../docs/reverse-engineering/protocol/perception-fusion.md) world-model of people, `user_alarm()`/`time_zone_info()` for [wake alarms + timezone](../../docs/reverse-engineering/protocol/power-and-system-events.md), `attention_classes()` for the [published attention state](../../docs/reverse-engineering/runtime/gaze-and-attention.md), and `volume_modify()`/`slow_input()`/`chatbot_listening()`/`allow_cutoff()`/`brain_reset()` for [imperative runtime control](../../docs/reverse-engineering/protocol/runtime-control.md), `wifi_app_status_classes()` for the [bo-wifi setup-app status](../../docs/reverse-engineering/protocol/qr-commands.md) (WifiAppReady/Bricked), `mpu_handling_classes()` for the [IMU handling events](../../docs/reverse-engineering/hardware/hardware-map.md) (pickup/shake/tilt), and the [MAINAPP interface](../../docs/reverse-engineering/protocol/unity-mainapp-interface.md) helpers `robot_camera()`/`cloud_tts_response()`/`user_pairing_request()`/`mainapp_lifecycle_classes()`/`audio_notif_classes()`. `pip install pyzmq`; tunnel via `adb forward`. |
...
> [`../../docs/reverse-engineering/ota-and-recovery.md`](../../docs/reverse-engineering/firmware/ota-and-recovery.md)).
> These generators are validated against the firmware's parser; end-to-end hardware validation is tracked separately.

---
📖 [Reverse-engineering](../../docs/reverse-engineering/README.md) · [QR commands](../../docs/reverse-engineering/protocol/qr-commands.md)
