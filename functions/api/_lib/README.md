...
| [`limits.js`](limits.js) | Request admission in one function: origin pin → per-IP windows → unit budget → concurrency ceiling → the bounded FIFO. Plus the Cache API tier — since 2026-09-06 it carries **every** per-IP window (minute, hour, day) and **both** unit-budget ceilings (hour, day), which is per-colo, defeated by a burst, and **fails open by construction** — every error is an undercount. |
