...
- **No *public* root exists** — but the boot chain has a concrete bypass: on AVB failure U-Boot drops
  ...
  `vbmeta` and then any `system`/`vendor`. See [`hardware-access.md`](../docs/reverse-engineering/hardware-access.md)
  and [`flashing-runbook.md`](../docs/reverse-engineering/flashing-runbook.md).
- **ADB** on retail units is locked (`ro.adb.secure=1`, `ro.debuggable=0`); **recovery-mode sideload
  and maskrom/rockusb bypass it** — see [`hardware-access.md`](../docs/reverse-engineering/hardware-access.md).
...
- **Pre-801 firmware has no custom-endpoint support** (endpoint pinned to `mqtt.googleapis.com`, CA-
  validated — [`network-trust.md`](../docs/reverse-engineering/network-trust.md)), so software re-home
...
- **Teardown artifacts** — UART pad map, maskrom test-point, per-partition read-back vs
  [hashes](../docs/reverse-engineering/firmware-803-reference.md). External teardown footage + FCC
...
---
📖 [Back to hardware](README.md) · [Field guide](../docs/reverse-engineering/FIELD-GUIDE.md) · [Hardware access](../docs/reverse-engineering/hardware-access.md)
