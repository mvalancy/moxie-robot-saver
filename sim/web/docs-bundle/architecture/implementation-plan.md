...
**Most valuable next slice (2026-09-03, re-ranked against the owner's headline goal — a live Sim demo on Cloudflare):** ① **the voice + listening picker** (`backlog/voice-picker.md`, build-ready) — the owner asked for it by name (*"allow users to pick from the available TTS and STT models, but default to piper-amy when possible — drop down for speech and listening"*), it is user-visible on the demo page, and local engines must stay first-class under it; ② **the unreachable child voice** — `audio.js`:160 `speak(text, who)` accepts a `who` and **no caller ever passes `"child"`**, so two committed clips are dead weight and the demo speaks with one voice where it was designed for two: a small, provable, visible fix; ③ **content packs export / import-with-review** (ADOPT #5, `backlog/content-packs.md`, P0 shippable alone); ④ **sandboxed content extensions** (`backlog/sandboxed-extensions.md` P0 — the server-side template-injection hole is already closed, this is the durable model); ⑤ **broker auth** (`backlog/security-broker-auth.md` P1, blocked on assumptions A1–A4). **Owner-blocked, not agent-blocked:** the three Cloudflare Production variables (`DEMO_GATEWAY_BASE_URL`, `DEMO_GATEWAY_API_KEY` as a secret, `DEMO_CHAT_MODEL`) — everything else in the hosted chain is now proven on a real preview deploy.
