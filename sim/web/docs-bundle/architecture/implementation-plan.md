...
| Hosted static site — what it admits it can do | [static-experience](static-experience.md) · [brief](backlog/live-sim-demo.md) | 🟡 **P0-a built 2026-09-02, unverified on a real Pages deploy**: the page no longer decides from the HOSTNAME. `sim/web/mode.js` polls same-origin `GET /api/health` (a Pages Function that makes **no gateway call, ever**, and is always 200 so a non-200 unambiguously means *route absent*) and `sim/web/env.js` paints the badge, a capacity pill, the `needs-backend` marks and the banner from the answer — `offline` (route absent) is **byte-identical to the site as it shipped**, `degraded` adds the honest reason, `live` stops claiming the mic needs a locally-run server. With no variables set at all the answer is `gateway_not_configured`, so a keyless branch preview is automatically the scripted demo and fires exactly **one** request per session. No secret is involved anywhere in this slice: `functions/api/_lib/envelope.js` builds every body from a fixed key allowlist, so no gateway URL, key or model id can ride along, and `sim/test_mode.mjs` asserts that over the raw text and every header. **Honest ceiling:** P0-a ships no live transport, so a configured deployment deliberately reads `SCRIPTED` and says why; the live turn (`/api/chat`, `/api/speech`, `cloud-transport.js`) is P0-b | `functions/api/health.js` + `functions/api/_lib/{env,envelope}.js` + `sim/web/mode.js` + `sim/web/env.js` + `sim/web/_headers` |
...
- **the hosted mode machine is proven hermetically and in a real browser, but three things
  ...
  answers in §10. A second, unrelated CI honesty note found while wiring this slice:
