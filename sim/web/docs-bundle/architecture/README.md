...
- [`backlog/`](backlog/README.md) — **the build briefs**: a ranked audit line turned into something an agent
  ...
  [`backlog/vendor-the-readme-hero.md`](backlog/vendor-the-readme-hero.md) (the one defect standing
  between the live site and a clean browser console — the README's hero image is hosted on
  `github.com`, which `img-src 'self' data: blob:` correctly refuses; the fix is to vendor the
  picture rather than widen the policy for it, and the guard against the next one is worth more than
  the image). One page there is the odd one out and
  deliberately so:
