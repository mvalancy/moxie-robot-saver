...
| 4 | **Memory as a product** | Beyond `complete_handler` writing a summary string: structured long-term memory (people, preferences, ongoing stories, goals) with summarization, decay, and a **parent-editable memory browser** including per-child deletion — because a memory a parent cannot read or erase is not acceptable on a child's device. | **L** | 🟡 **the floor shipped 2026-09-02** — memory *is* structured (facts / preferences / open_threads / summaries, module-namespaced, bounded, **provenance on every item** per Fork A's `conversation_memory.py`), written by `session.summarize()` at end-of-conversation, gated by `LoggingPolicy.NO_DATA`, and **readable + erasable by a parent** over `GET`/`DELETE /memory`. The **browser UI shipped 2026-09-02**: the console's 🧠 What Moxie remembers card lists every item per activity with the date, module and turn count it came from, erases one activity or everything behind a two-click confirm, states the `LoggingPolicy` when remembering is off, and says plainly that a summary can be wrong (`normalize_memory` + `GET`/`DELETE /local/robots/{id}/memory[/{namespace}]`, [what-moxie-remembers.md](../guides/what-moxie-remembers.md)). Still 🟡, not 🟢: **per-item erase and editing** (the runtime erases per namespace or all — the UI offers exactly that and no more), per-item provenance (it is recorded per merge), and **decay** — the caps are a blunt substitute |
