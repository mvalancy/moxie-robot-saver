...
| Puppet / telehealth | full: `moxie_mode:"TELEHEALTH"`, `PLAY_OUTPUT`/`INTERRUPT`, mood+intensity picker, live state poll | `MoxieMode.TELEHEALTH` exists in `cloud_config.py`; **no command path, no UI** | **ADOPT** (M) — 🔵 **brief ready**: [`backlog/telehealth.md`](backlog/telehealth.md) |
...
| **Face customization** | full layered editor + ~60-asset table + "new child ID" cache-buster | **SHIPPED** — `child_pii.face_options` + a deterministic `child_pii.id` cache-buster, fleet ⊕ per-robot layering, the console's 🎨 Moxie's look card; catalog is the **12 doc-cited colour options across 2 of the 14 recovered slots**, with no invented ids (see ADOPT #9) | **HAVE** (thinner catalog, honest about it) |
...
| 7 | **Puppet / telehealth mode + a console page** | `moxie_mode:"TELEHEALTH"` + `PLAY_OUTPUT`/`INTERRUPT` makes Moxie a telepresence body — the single best demo, and a real accessibility feature. We already have the enum; we need the command path and a page. | `views.py::puppet_api`, `moxie_server.py::send_telehealth*`, `templates/hive/puppet.html` | **M** | 🔵 **open — brief ready.** [`backlog/telehealth.md`](backlog/telehealth.md): a pure `moxie_sdk/telehealth.py` (JSON shaped by the recovered `TeleHealth.proto`, cross-checked against `TeleHealth_pb2`), six runtime methods + `GET`/`POST /telehealth` in the status-HTTP region, the 🎭 "Be Moxie" console card (11 recovered moods, intensity 0–2, interrupt, live transcript), and a `bridge.js` handler so the SIM speaks the operator's line in CI. Operator text goes through the safety classifier — a block is **returned to the operator**, never silently rewritten. |
...
| [`backlog/telehealth.md`](backlog/telehealth.md) | **ADOPT #7** (puppet / telehealth) — the `commands/telehealth` command path and the 🎭 "Be Moxie" console panel | 🔵 **ready to build** |
