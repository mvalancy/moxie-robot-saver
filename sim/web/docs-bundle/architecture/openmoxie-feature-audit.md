...
| [`backlog/qr-launch-cards.md`](backlog/qr-launch-cards.md) | **§4.4 #9** (printable launch cards) — the row **re-scoped**: arming the runtime QR reader (`eb_enable_qr` as an `execute` action), routing `$eb_qr_value` into a real launch against a closed 24-id allowlist, *then* the sheet a parent prints | 🟢 **build-ready — spec'd 2026-09-03.** Written because the ranked row was **wrong, not stale**: it priced an **S** on a parser that only ever reads a brain's own reply text. **M**, three pieces. Honest ceiling: the end-to-end claim is unprovable on hardware — no physical robot has ever sent us a vision event |
...
**Not ranked, deliberately — and the list grew.** BEYOND #9 (vision events), **§4.1** ADOPT #9's two
flagged assumptions (the *face catalog* — not the launch cards ranked #9 above; the two numbering
schemes collide and this line used to read as though they were one item), the launch cards' own Q1/Q2
(*is `eb_enable_qr` the lever that arms the runtime reader, and when should it be armed* —
[`backlog/qr-launch-cards.md`](backlog/qr-launch-cards.md) §7), the `wakeup` acknowledgement, broker
auth A1–A4, every claim in the OTA row, and — new on
