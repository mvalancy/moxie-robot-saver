...
| [`backlog/production-hardening.md`](backlog/production-hardening.md) | **§4.4 #3** (production hardening) — the cross-process store decision, MQTT reconnection, and a soak that stands in for a week | 🟢 **P0 shipped 2026-09-03; P1/P2 build-ready.** The brief existed to remove one owed decision and did: §3 chose advisory `flock` on a sidecar behind `JsonStore.transaction()` over WAL-SQLite and over a single-writer rule, with the comparison table, the defence and the trigger that would flip it — **and the build hit none of the falsifiers**, so the decision stands as written. P0 built §3 and §4 (`connect_async` + `retry_first_connection=True` + a checked CONNACK + eight checked publishes) with 40 new tests and 35 mutations, 0 missed. **Still open: all of §5** — `run_soak.sh --profile week` and its 11 numeric bars do not exist, so *"stays connected for a week"* is a claim nothing in this repo has measured — plus the durable roster, connection telemetry and the SIGTERM handler. P1 **M** · P2 **L**. **Six of its twenty-one assumptions need a physical robot, and P0 did not move that number** |
