...
| [`backlog/production-hardening.md`](backlog/production-hardening.md) | **§4.4 #3** (production hardening) — the cross-process store decision, MQTT reconnection, and a soak that stands in for a week | 🟢 **build-ready, new 2026-09-03.** The brief exists to remove one owed decision and does: §3 chooses advisory `flock` on a sidecar behind `JsonStore.transaction()` over WAL-SQLite and over a single-writer rule, with the comparison table, the defence and the trigger that would flip it. §4 (`connect_async` + `retry_first_connection=True` + a checked CONNACK + eight checked publishes) · §5 (`run_soak.sh --profile week`, 11 numeric bars). P0 **M** · P1 **M** · P2 **L**. **Six of its twenty assumptions need a physical robot** |
...
| 3 | **Production hardening for a robot that stays connected** (Fork A's list) | ① | 🟢 **build-ready** *(promoted from 🟠 on 2026-09-03 — the owed decision is made)* | The two defects are unchanged and re-verified: [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):**484** — the line moved from :458, the problem did not — is still a plain blocking `client.connect(self.host, self.port, 30)` with no `connect_async` and no `reconnect_delay_set`, and [`store.py`](../../mqtt/moxie_sdk/store.py):71's lock is in-process only across what is now **fourteen** collections (`ext_events` landed with the extensions slice the same day). Every one is a bug we have not hit because no real robot has been on our broker for a week — and outcome ① is measured live. **The 🟠 was owed to the cross-process design decision; [`production-hardening.md`](backlog/production-hardening.md) §3 makes it** — advisory `flock` on a per-record sidecar behind a public `JsonStore.transaction()`, JSON staying on disk, over WAL-SQLite and over a single-writer rule, with the comparison table and the falsifier written down (*"SQLite's only real advantage here is multi-collection transactions, and not one of our fourteen call sites uses one"*). §4 settles reconnection down to the trap that `connect_async` without `loop_forever(retry_first_connection=True)` **changes nothing**, and decides what an in-flight turn does across a reconnect (abandoned via the existing `_turn_seq` guard, never replayed — a chunk 1 landing after the robot's ~20 s re-prompt would answer the question the child gave up on). §5 turns *"a week"* into `run_soak.sh --profile week` with eleven numeric bars. **No design decision is owed; an agent can open a worktree.** Honest ceiling unchanged, and stated in the brief's §0 and §5.4 rather than a footnote: six of its twenty assumptions need a **physical robot**, so P0/P1 prove our half of every failure and nothing proves a week in a house |
