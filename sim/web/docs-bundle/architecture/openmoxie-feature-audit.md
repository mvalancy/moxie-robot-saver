...
|---|---|---|
| [`backlog/expressiveness.md`](backlog/expressiveness.md) | **ADOPT #3** (the markup floor) §1 · **BEYOND #1** (the behavior planner) §2 | §1 🟢 **shipped 2026-09-02** · §2 P1 🟢 **shipped 2026-09-03** (P2 open) |
...
| 4 | **The behavior planner** (BEYOND #1) | ③ | 🟢 **P1 shipped 2026-09-03** (P2 open) | Built to the brief: `moxie_sdk/performance.py` scores a line's **dialog act** (all 22) and stages a validated `Performance` that exactly one `render()` turns into markup, behind the same `make_markup` seam — so a question tilts and holds its gaze, an apology stops gesturing, praise celebrates, and an "mm-hm" moves nothing at all. Contract changes C1–C7 landed, so `mood`/`mood_intensity`/`dialog_act`/`emotion`/`signals` are filled on **every** published turn including streamed chunks (they were plumbed and empty since PR #17), and `POST /local/robots/{id}/preview` rehearses a line on the SIM before a child hears it. Deterministic, no model call, p95 0.25–0.56 ms; a planner failure always degrades to the floor (`MOXIE_EXPRESSIVE=planner\|floor\|off`). **P2 — a learned/model-assisted scorer, and the honest answer to their ML rule table — is still open.** Brief: [`expressiveness.md`](backlog/expressiveness.md) §2 |
=======
