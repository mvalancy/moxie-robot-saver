...
[`perception-pipeline.md`](perception-pipeline.md)), `android.hardware.location*` / `sensor.*` (no GPS,
...
> These map one-to-one onto the [`hardware-map.md`](hardware-map.md) device list — the policy is the
...
`ro.oem_unlock_supported=1` unlock + resign path (see [`hardware-access.md`](hardware-access.md)).
...
TLS/endpoint story in [`network-trust.md`](network-trust.md) / [`cloud-protocol.md`](cloud-protocol.md)).
...
---
📖 [Reverse-engineering index](README.md) · [Field guide](FIELD-GUIDE.md) · [Firmware reference](firmware-803-reference.md) · [Hardware map](hardware-map.md)
