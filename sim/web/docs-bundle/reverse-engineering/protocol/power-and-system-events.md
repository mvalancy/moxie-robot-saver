...
> It's the proto layer beneath the app-level [`boot-and-launcher.md`](../firmware/boot-and-launcher.md) (which
...
| # | `State` | Meaning (see [boot-and-launcher](../firmware/boot-and-launcher.md)) |
|--:|---|---|
...
- **`PowerStayAwakePB { bool busy }`** — the keep-awake pulse: while `busy` is set (an activity/upload in
  ...
  `ReloadQueueStayAwakePulseEvent` → `PowerStayAwakePB`, see [`behavior-input-events.md`](../runtime/behavior-input-events.md).)
...
co-processor, [`hal-and-drivers.md`](../firmware/hal-and-drivers.md)). `WifiRecoverRequest` similarly kicks the Wi-Fi
stack.
...
| **`OTAStatus`** | `update_status`, `payload_complete`, `update_percent`, `payload_result` | live OTA progress ([`ota-and-recovery.md`](../firmware/ota-and-recovery.md)) |
