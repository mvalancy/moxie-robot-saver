...
> turns raw percepts — [face/body/QR detections](../runtime/perception-pipeline.md) and [audio DOA/STT](../runtime/perception-pipeline.md#audio-hear-understand-speak) —
...
> actually reasons over — the input beneath [gaze](../runtime/gaze-and-attention.md) and
> [turn-taking](../runtime/turn-taking.md), and it arrives on the bus as `FusedPeopleEvent`
...
| `name`, `fullname` | recognized identity (from [face recognition / enrollment](../runtime/perception-pipeline.md#face-recognition-enrollment-data-model)); empty if unknown |
...
responsive turn-taking ([turn-taking](../runtime/turn-taking.md)).
...
- **World** (`world_x/y/z`, `world_width/height`) — 3D, robot-relative. This is what
  [gaze & attention](../runtime/gaze-and-attention.md) consumes to build interest points and drive the IK look-at:
