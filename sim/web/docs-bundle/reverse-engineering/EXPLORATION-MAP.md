...
| ✅ | `embodied.robotbrain` (100) | 100/100 | [remote-chat-protocol](protocol/remote-chat-protocol.md), [content-and-conversation](runtime/content-and-conversation.md), [runtime-control](protocol/runtime-control.md), [gaze-and-attention](runtime/gaze-and-attention.md) (MentorAction/EndedReason taxonomy + residue) |
| 🟡 | `embodied.logging` (55) | 40/55 | [device-config-and-telemetry](protocol/device-config-and-telemetry.md), [cloud-protocol](protocol/cloud-protocol.md) (CloudQuery API + **built-in endpoint-host table** for all 7 IOTEndpoint profiles incl. `EMBODIED_LOCAL`→`client-service-api.local` + `cloud.json` persistence, via Ghidra/strings on `libbo-logger`) — residue: RobotCloudConfig field-messages + small pairing/report stubs |
