...
[perception-pipeline](perception-pipeline.md#output-side-tts-embodiedunity)) is the core:
...
| `AudioIsFinishedEventPB` | — | the current clip finished ([behavior-input-events](behavior-input-events.md)) |
...
| `SpeechPlaybackState` | — | speech-playback state ([perception-pipeline](perception-pipeline.md)) |
...
the mic — the audio counterpart of the [`MpuIsNoisy` gate](hardware-map.md#semantic-handling-events-embodiedunity).
...
| Message | Payload | Meaning |
|---|---|---|
| `EngagedEvent` | `engaged` (bool) | engagement crossed on/off (cf. the [fused engagement](perception-fusion.md) + [turn-taking](turn-taking.md)) |
| `RobotEngageTurn` | `turning` (bool) | Moxie is turning to engage a target ([perception-pipeline](perception-pipeline.md)) |
...
[unity-assets](unity-assets.md); delivery in [content-delivery](content-delivery.md)) at runtime:
...
So new/updated content ([content-delivery](content-delivery.md)) is applied to the live face by a
...
- **`PairingRequest`** is the full action set: `PAIR`, three flavours of unpair (`UNPAIR_USER`,
  ...
  ([crypto-and-keys](crypto-and-keys.md)); `UserDataStatus{code}` reports the flow's result.
...
| `NetworkState` | `Connected`, `Ping` | the MAINAPP's view of connectivity ([behavior-input-events](behavior-input-events.md)) |
...
- **`MarkUpToolMessages`** — the in-house **behavior-markup authoring tool** (a designer edits a line's
  ...
  [behavior-markup](behavior-markup.md) grammar.
- **`Gaze`** — the gaze target the MAINAPP renders, driven by the attention system
  ([gaze-and-attention](gaze-and-attention.md)).
- **`MpuPickup`** — IMU handling events surfaced through the Unity layer
  ([hardware-map](hardware-map.md#semantic-handling-events-embodiedunity)).
...
lip-syncs to ([perception-pipeline](perception-pipeline.md)). Pairing (`UserPairingRequest`) rides the
...
---
📖 [Reverse-engineering index](README.md) · [Robot IPC protocol](robot-ipc-protocol.md) · [Perception pipeline](perception-pipeline.md) · [Unity assets](unity-assets.md) · [Content delivery](content-delivery.md) · [Behavior markup](behavior-markup.md) · [Gaze & attention](gaze-and-attention.md)
