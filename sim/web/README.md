...
| `_headers` | Cloudflare Pages cache policy **and** the site's security headers (`/api/*` `no-store`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`, **HSTS**, and a CSP whose `connect-src 'self'` and `script-src 'self'` are the real controls). `script-src` names exactly one off-origin host — `https://static.cloudflareinsights.com`, the analytics beacon **Pages injects** into every HTML response, which the policy otherwise refused on every page load; the file explains why `connect-src` did **not** have to widen with it, and why the inert `/api/*` block must never grow security headers (they live in `envelope.js`). `'unsafe-inline'` for scripts stays until the fourteen inline blocks (four importmaps) and ten inline event-handler attributes are hashed or moved — the specific blocker is written down in the file. **It applies to these static pages only, never to Pages *Function* responses** (assumption-ledger row 27). Exercised for real by `sim/test_csp.mjs`, which serves every page with this file's headers actually applied |
...
| `rail.js` | sim.html: the phone-width rail drawer, so the 3D stage stays visible |
