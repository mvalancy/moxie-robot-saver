...
| `_headers` | Cloudflare Pages cache policy **and** the site's security headers (`/api/*` `no-store`, `nosniff`, `Referrer-Policy`, `Permissions-Policy`, **HSTS**, and a CSP whose `connect-src 'self'` and `script-src 'self'` are the real controls). `'unsafe-inline'` for scripts stays until the fourteen inline blocks (four importmaps) and ten inline event-handler attributes are hashed or moved — the specific blocker is written down in the file. **It applies to these static pages only, never to Pages *Function* responses** (assumption-ledger row 27). Exercised for real by `sim/test_csp.mjs`, which serves every page with this file's headers actually applied |
