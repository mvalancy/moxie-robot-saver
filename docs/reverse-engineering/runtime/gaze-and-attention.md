...
> [`hardware-map.md`](../hardware/hardware-map.md) (head + eye motion goes out), and is aimed/gated by the gaze
...
- **`Attention.targeted_user`** / **`TargetedUser.targeted_user_id`** is a **`FusedPerson.id`** from
  [perception fusion](../protocol/perception-fusion.md#fusedpersonpb-one-tracked-human) — so the attention decision
  names a *specific tracked person*, and `targeted_user_face_id` ties to that person's
  [`FusedFacePB.face_tracker_id`](../protocol/perception-fusion.md#fusedfacepb-the-face-in-two-coordinate-frames).
...
So the pipeline is **[fusion](../protocol/perception-fusion.md) (who's in the room) → `Attention` (who I'm attending
...
3. **IK look-at** — `EBAnimIKLookAtHandler` drives the head (motor 4) + body-yaw (motor 5, see
   [`hardware-map.md`](../hardware/hardware-map.md)) and the **projected eye pupils** toward the goal, with the
...
server (or a [telehealth](../protocol/telehealth.md) operator, or the parent app) **can subscribe to the published
`Attention`** to know *who* Moxie is attending to (a [fused-person](../protocol/perception-fusion.md) id) and its
