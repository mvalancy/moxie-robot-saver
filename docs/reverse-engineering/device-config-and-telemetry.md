...
| **Device** | `audio_volume`, `screen_brightness`, `timezone_id`, `settings` (`DeviceSettings` k/v — cf. [settings-schema](settings-schema.md)) |
...
- **`DeviceSettings{ props[]: {key,value} }`** / `DeviceSettingsUpdate` — the flat settings bag echoed in
  both `RobotCloudConfig` and `RobotStatus` (schema in [settings-schema](settings-schema.md)).
...
---
📖 [Reverse-engineering index](README.md) · [Cloud protocol](cloud-protocol.md) · [Crypto & keys](crypto-and-keys.md) · [Power & system events](power-and-system-events.md) · [Settings schema](settings-schema.md) · [Content & conversation](content-and-conversation.md)
