...
The local [ChatScript engine](content-and-conversation.md) reporting up: **`ChatScriptReady`** when it's
...
---
📖 [Reverse-engineering index](README.md) · [Settings schema](settings-schema.md) · [Turn-taking](turn-taking.md) · [Robot IPC protocol](robot-ipc-protocol.md) · [Power & system events](power-and-system-events.md) · [Device config & telemetry](device-config-and-telemetry.md)
