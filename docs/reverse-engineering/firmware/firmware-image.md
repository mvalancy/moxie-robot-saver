...
| App (priv-app) | Role |
|---|---|
| **`bo-android`** | **The "brain."** Main experience app — conversation, vision, behavior, Unity face. See [`robot-ipc-protocol.md`](../protocol/robot-ipc-protocol.md). |
| **`bo-wifi`** | **The setup/"Wifi App."** Unity app that scans setup/pairing/**debug** QR codes, joins Wi-Fi, drives pairing. See [`qr-commands.md`](../protocol/qr-commands.md). |
...
See [`hardware-map.md`](../hardware/hardware-map.md) for the motor/sensor/LED enumeration recovered from the
...
- **For custom firmware (the big lever):** because privileged grants come from priv-app *placement*
  ...
  [`hardware-access.md`](../hardware/hardware-access.md)); once you can do that, the app layer imposes no extra
...
3. **Replace the experience, not the hardware layer.** Swap `bo-android`/`bo-wifi` for your own APK
   that speaks the same **ZMQ + protobuf** IPC (see [`robot-ipc-protocol.md`](../protocol/robot-ipc-protocol.md))
