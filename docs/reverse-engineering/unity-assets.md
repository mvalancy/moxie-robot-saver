...
| Scripting | Mono (`libmonobdwgc-2.0.so`); managed `Assembly-CSharp.dll` (see [`robot-ipc-protocol.md`](robot-ipc-protocol.md)) |
...
- **Status LEDs (animator `ledBarMoxieAnimator` / `ledSpriteAnimator`):** `LED_turningOn`,
  ...
  states (mirror the MCU `LedrPattern`, see [`hardware-map.md`](hardware-map.md)).
- **Reward / STAR system (`RewardStarAnimator`):** vortex/heart/explosion sprite sequences —
  the mentorship reward animation (`reward-star` markup verb, [`behavior-markup.md`](behavior-markup.md)).
...
- **Dev console:** a `EB Console Window` / `EB OSA Layout Entry` GameObject — the on-device console
  (`embodied.Robot.ConsoleCommandRequest`, see [`robot-ipc-protocol.md`](robot-ipc-protocol.md)).
...
`oem` — see [`hardware-access.md`](hardware-access.md)).
