...
- Your images (a genuine 803 set, or your own `system`/`vendor`/`boot`) — partitions + hashes in
  [firmware-803-reference](firmware-803-reference.md); flash targets in [hardware-access](hardware-access.md#partition-table-flash-targets).
...
re-home](FIELD-GUIDE.md#-revive-an-old-robot) + [self-hosted server](cloud-protocol.md) apply. This is
...
---
📖 [Hardware access](hardware-access.md) · [Firmware image](firmware-image.md) · [Field guide](FIELD-GUIDE.md) · [Coverage](COVERAGE.md) · [Docs index](../README.md)
