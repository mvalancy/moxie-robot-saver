# 🔊 TTS on the LiteLLM gateway — setup handoff

**For:** the LiteLLM gateway manager.
**Goal:** register **one TTS model** on the gateway so `POST /v1/audio/speech` works with the existing
API key. Then the Moxie cloud server speaks through the **same gateway, same key, same rate limits** as
chat — no separate voice service or key needed.
...
Just this and we're wired in (we set `MOXIE_VOICE_BASE_URL=https://gateway.graphlings.net/v1` + the
