...
`__import__` of `os` and a `getcwd` call execute arbitrary Python — we ran exactly that probe
against `render_prompt` with a real `Volley`, and it returned the process working directory.
Template injection through a `prompt` is server-side code execution.
