...
| [`production-hardening.md`](production-hardening.md) | **§4.4 #3** — production hardening: the cross-process store story (a decision ADOPT #8 deferred three times), MQTT reconnection, and *"stays connected for a week"* as a test we can run without hardware | M · M · L | 🟢 **build-ready, new 2026-09-03.** §3 **decides** the owed question — advisory `flock` on a per-record sidecar behind a public `JsonStore.transaction()`, JSON staying on disk — over WAL-SQLite and over a single-writer rule, and names what would flip it. §4 settles reconnection (including the trap that `connect_async` without `loop_forever(retry_first_connection=True)` changes nothing) and what an in-flight turn does across a reconnect. §5 is `run_soak.sh --profile week` with 11 numeric bars. **Six of its twenty assumptions need a physical robot** — the honest ceiling on this whole area |
