...
| Browser tier — the machine-speed defect family | [sil-and-cicd](sil-and-cicd.md) | 🟡 **swept, three instances measured and fixed, the rest cleared honestly (2026-09-06).** The family: *an assertion that compares a live sample against recorded state, or two live samples at different instants* — green on a laptop, red on a loaded runner, and it reads as a flake. `page_teeth_check.py` breaks the SITE; it could not see this half, so it grew **`--slow N`**, which throttles the RENDERER over CDP (`Emulation.setCPUThrottlingRate`; measured 10.3 ms → 58.5 ms at 6x on an in-page busy loop) while node's timers keep full speed — the laptop/runner asymmetry, created rather than waited for. Found: `test_env_hosted.mjs`'s single `setTimeout(3000)` decided ~45 assertions (**14 of 45 fail, 3/3 runs**, every one reading `state: "boot"`, when `/api/health` answers in 4.5 s — inside the product's own 6 s `PROBE_TIMEOUT_MS`); its `probed ONCE and never again` check cannot see a poll storm (the poll floor is 30 s, no affordable settle reaches it — proved by deleting `mode.js`'s sticky guard and watching the check stay green); `test_csp.mjs`'s `search filters the tree` at a fixed 1200 ms against a **3.27 MB** corpus fetched on the first keystroke (measured: typed t=12.0 s, tree filtered **t=24.5 s** at 400 KB/s); `test_csp.mjs`'s CSSOM refutation counting **239** style-src violations that belonged to mermaid, at 20x. **0 red after, same instruments.** Cleared by measurement, not by reading: `test_mermaid`, `test_a11y` (79 checks), `test_api_headers` (382), `test_console_insights` (262), `test_mic_spend` (54) and `test_responsive` all survive 20x; and `test_csp`'s page-readiness probe is clean for a reason (module scripts block `domcontentloaded`, so the 2500 ms sleep is not what makes it true). **NOT closed, and measured saying so:** at load average 118 on the author's box `test_env_hosted` still fails 2 of 3 runs — but on the *other* side of the same seam. The fix removed the sampling race (it now reads `offline`, a real verdict, where the old code read `boot`, no verdict at all); what remains is that puppeteer's request interception answers `/api/health` from **node**, so a starved event loop can miss `mode.js`'s own 6 s `PROBE_TIMEOUT_MS` and the page correctly decides offline. Unfixed here: it needs the fixture's reply off the node event loop, not another wait. `test_mobile_layout`, `test_typed_turn` and `test_ambient_guard` at 20x fail by their own bounded waits EXPIRING (a harness-timeout signature, not a stale sample); a clean 6x re-read was not obtainable while the box sat at load 120 | `sim/tools/page_teeth_check.py` (`--slow`) · `sim/tools/teeth_ledger.mjs` · `sim/tests/test_page_teeth_slow_mode.py` · `sim/test_env_hosted.mjs` · `sim/test_csp.mjs` |
...
~~③ **A TTS cache**~~ — **SHIPPED as PR #152; this entry was stale.** Verified against the code on 2026-09-06, not against a claim: [`functions/api/_lib/ttscache.js`](../../functions/api/_lib/ttscache.js) exists and [`speech.js`](../../functions/api/speech.js) consults it in five places. The sentence it replaces — *"the demo re-synthesises identical phrases on every visit"* — had been untrue for a day while still reading as the cheapest available win, which is precisely how an agent gets sent to rebuild something that already works. ~~④ **The hour/day windows and the unit budget on the Cache API tier**~~ — **the UNIT BUDGET half shipped 2026-09-05** ([`live-sim-demo.md` §4.6.2](backlog/live-sim-demo.md)): the deployment's hourly spend is now counted per-colo alongside the minute window PR #146 shared, so the *spend* ceiling is no longer the loosest of the four. It is **not** the same change made twice — a budget can be refunded and a window cannot, and a lost refund on an eventually-consistent counter fails **closed** (it refuses visitors who should be served), which is the direction §4.6.1 had already rejected the concurrency ceiling over. So the shared entry is never told about a charge until the request has been released *without* a refund; there is no refund write to lose. ~~What remains of ④ is the **per-IP hour/day windows** and the unit budget's **day** ceiling, both still one isolate's `Map`.~~ 🩹 **THAT REMAINDER SHIPPED 2026-09-06 — ④ IS NOW CLOSED IN FULL, verified against the code and not against a claim.** [`functions/api/_lib/limits.js`](../../functions/api/_lib/limits.js):20-22 states it in the file header, and the three functions are readable: `sharedWideWindow` :1638 (the per-IP **hour and day** windows, sharing ONE cache entry so the two scales cost one round trip between them — the latency objection this plan recorded was *answered*, not overruled, and :1229-1236 keeps the original objection standing beside the answer), `sharedBudgetVerdict` :1811 (the unit budget's hour) and `sharedDayBudget` :1933 (its day twin, on its own ledger because hour and day roll on different clocks). **What is deliberately still per-isolate**, so this paragraph cannot rot in the other direction: the **concurrency ceiling** and its FIFO — a lost give-back leaks a slot for ever and fails **closed** — plus `/api/health`'s probe (:41-45). And they remain **per-colo, never global** (:51-55). ⑤ **`'unsafe-inline'` in `style-src`** — a much smaller hole than the script one closed in #137, and honestly labelled as such.
...
**Re-verified as ALREADY BUILT on 2026-09-04/05, do not brief:** the child voice; sandboxed extensions P0; the behavior planner; content packs; the voice + listening pickers; durable telemetry; the Cache API minute-window tier (#146, proven on production); the QR decoder (#148), its wire round trip (#149) and its browser parity (#151). **Owner-blocked, not agent-blocked:** whether KV / Durable Objects are offered on this Cloudflare plan (§10 assumption 13's dashboard half). *(Turnstile's keys were the other entry on this line and are now set — the control shipped 2026-09-05.)*

- **⏱️ Two of the five above were overtaken within hours of being written (audit refresh, 2026-09-05) — read the paragraph with this bullet, do not edit the paragraph.** ③ **the TTS cache SHIPPED** as **PR #152** ([`functions/api/_lib/ttscache.js`](../../functions/api/_lib/ttscache.js); it sits *after* every cap, so a hit can never be a way past `DEMO_MAX_TTS_CHARS`, the origin pin or the per-IP window). ① **was narrowed, not closed,** by **PR #153**: real spoken words now go through the shipped `/api/transcribe` both via `onRequestPost` against the real gateway and by POSTing the same bytes at a real deployment — **verbatim, word overlap 1.00**, against a decoy control of 0.07 — so *the route* is proven, while the literal claim stands that **no human has held a browser microphone open on the hosted page**. ~~That leaves ④ (the hour/day windows and the unit budget on the Cache API tier) as **the highest live item on this list that is neither shipped nor owner-blocked**, and it is now the audit's own ⭐ pick~~ — [`openmoxie-feature-audit.md` §4.4](openmoxie-feature-audit.md#-the-single-most-valuable-next-slice-the-spend-ceilings-on-the-cache-api-tier-shipped). 🩹 **Overtaken again, 2026-09-06: ④ shipped in full** (see its own entry above for the three functions), **and ② shipped too** — Turnstile is no longer owner-blocked or in flight: [`functions/api/_lib/turnstile.js`](../../functions/api/_lib/turnstile.js) (511 lines) is called from [`chat.js`](../../functions/api/chat.js):199, after every free refusal and immediately before the only `fetch()`, with the browser half at [`sim/web/turnstile.js`](../../sim/web/turnstile.js) and 1 986 lines of [`sim/test_turnstile.mjs`](../../sim/test_turnstile.mjs) behind it. **That leaves ⑤ (`'unsafe-inline'` in `style-src`) as the highest live item on this list that is neither shipped nor blocked** — still present at [`sim/web/_headers`](../../sim/web/_headers):247, and read that file's 2026-09-05 second-pass note at :163-233 first, because the *stated* blocker there was wrong once already. The dashboard half of assumption 13 remains owner-blocked. **Nothing here was re-ranked — every item above it simply shipped.**

① ~~**The child-privacy contract's last two gaps**~~ — **BOTH CLOSED; this entry contradicted [:1368](#) of this same file, which had already recorded them closed on 2026-09-04. Re-verified against the code 2026-09-05.** `do_DELETE` now accepts `/telemetry` as well as `/memory` ([`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):1191-1213), and the behaviour log is *not* ungated — it resolves `telemetry_policy`, and under `NO_DATA` the record is parsed and returned but never written. Erasure reaches it because `ACTIVITY_COLLECTIONS` names `MENTOR_BEHAVIORS_COLLECTION` alongside the packet ring and the daily rollup, so one `erase_telemetry` clears all three — never policy-gated, so a parent can erase without turning recording off. That list exists precisely so this cannot regress: *"One list, so a fourth activity record cannot be added without this erase being told about it."* ② **Live-Sim P1 remainder** — the per-IP and concurrency limits are **per-isolate**, and `functions/api/_lib/limits.js`:13-21 says so plainly: "a Cloudflare Worker isolate is not a shared counter". The documented per-colo Cache API tier was never implemented; Turnstile appeared nowhere in `functions/` or `sim/web/`; no TTS cache. *(All three closed since: the Cache API minute-window tier in #146, the TTS cache in #152, and the Turnstile bot control on 2026-09-05 — `functions/api/_lib/turnstile.js` + `sim/web/turnstile.js`. The per-IP hour/day windows and the unit budget are still one isolate's `Map`.)* ③ **The QR launch cards** ([`backlog/qr-launch-cards.md`](backlog/qr-launch-cards.md), banner reads 🟢 build-ready) — three server-side hops, none of them shipped. ④ **`'unsafe-inline'` in the page CSP** — still present at [`sim/web/_headers`](../../sim/web/_headers):29, honestly labelled "the honest gap" at :36; three inline blocks in `sim.html` and a static `_headers` cannot carry a nonce. ⑤ **Broker auth** ([`backlog/security-broker-auth.md`](backlog/security-broker-auth.md)) — genuinely blocked on assumptions A1-A4, not merely unstarted.
...
### Progress against the six — scored 2026-09-04, re-scored 2026-09-06
...
|---|---|---|---|
| 1 | A child can talk to Moxie end to end | 🟢 | **Restored 🟡→🟢 the same day, by running the thing the downgrade asked for.** The 🟡 above said *"🟡 until one run covers the whole chain"*, and named the seam exactly: `run_smoke.sh` pinned `MOXIE_APP=echo` with no flag, so five real layers (broker, supervisor process, TTS audio, virtual robot) surrounded a **mocked brain** answering `'You said: hello Moxie'`; `test_live_gateway.py` was the mirror image, a real brain *"minus the broker"* by its own docstring. `sim/run_smoke.sh --live-brain` is that run: on 2026-09-04 `MOXIE_SIL_PORT=1932 bash sim/run_smoke.sh --live-brain` drove broker + supervisor + runtime + **a live gateway brain** + TTS + virtual robot in **one process tree** — state→config(paired)→remote-chat→reply, the reply `"Hello there! I'm so happy to see you. How was your morning?"` written by a model on the gateway and *asserted not to be* the echo app's `You said: <prompt>` (`--reject-echo`, which is what stops a silent fallback to `echo` from passing as a live proof), with **207 196 B @ 22050 Hz** of real TTS audio round-tripped and all five scored fields on the wire. Dispatched by the deep tier, which **fails on its own SKIP line** so an absent secret cannot report a green live run. The default `sim/run_smoke.sh` is unchanged (echo app, no key, no gateway call), `run_scenarios.sh` is 2/2 with 4/4 turns each, and the hosted probe against `moxie.mattvalancy.com/sim` still exercises the Cloudflare path rather than this one — it closed nothing then and closes nothing now. Not a mock at any hop, brain included. **Named residual, 2026-09-08 — this chain has exactly one brain and no second path.** `gateway.graphlings.net` returned `503 no_db_connection` to every request for well over an hour; `functions/api/_lib/env.js` takes one `DEMO_GATEWAY_BASE_URL` and `chat.js` has no fallback provider, so one vendor's auth database is a total outage for the live demo. **The score is unchanged and deliberately so:** this row scores *proven capability*, and the 2026-09-04 run proved it; it does not score *present availability*, which was zero for that hour. The degradation path did hold — `mode=degraded`, `reason=upstream_down`, badge painted, one spoken line, scripted Moxie carrying the page, verified live during the outage rather than reasoned about. Unlike #4 this residual is **not** a ceiling: it is a second credential and a bounded retry, costed three ways in `docs/architecture/backlog/one-brain-no-failover.md`, and the cheapest option is *do nothing*. **Ceiling: no physical robot, ever, anywhere in this table** — the *child-in-the-room* half of this sentence is #4's amber and this row does not claim it; a live turn is one dispatched turn of `hello Moxie`, not a conversation. *(That restoration was folded into the total on 2026-09-06, when #6 was re-scored in the other direction — see the total below.)* |
...
| 6 | Green + tested | 🟡 | **A raw hermetic count is NOT the evidence this row long implied, and 2026-09-06 proved it: `origin/dev` measured 5229, 5334, 5347, 5459 and 5488 *on the same commits*, all correct, differing only by venv composition and by whether the tree was a git repo (a `git archive` copy shifts tests from passed to skipped without changing the sum). The stable quantity is `passed + skipped`. Build from `sim/tests/requirements.txt`, measure your own baseline, and compare only against that.** What actually moved this criterion on 2026-09-06 was **nine defects of one class — a check that passes against a system that is actually broken** — found and closed across six files: a baseline sampled before the state change it measured; an image read before decode; a fixed sleep standing in for a condition; an assertion matching a shipped `Loading docs…` placeholder (green against a page that had loaded nothing); three health numbers collected, printed and asserted nowhere; a dead `unaccrueDayPending()` branch a 151-check suite could not reach; five further assertions in that same suite that could not fail; an rAF instrument throwing `IndexSizeError` on every run, invisible until something listened; and a mutation tool silently degraded from *the right check reddened* to *something reddened*. Coverage is now **auditable on demand** rather than assumed: `sim/tools/page_teeth_check.py` answers "which checks stay green against a broken page?" (TIER A: 0 for `check_deployed`), `unit_budget_mutation_check.py` runs 35 rows over the shared ceilings, and five browser suites that installed no console listener at all can now see a 404, a CSP refusal or a thrown exception. Earlier figures, kept for the trend: 5 299 on 2026-09-05 after six merges; 5 164 earlier that day; 5 037 on 09-04, 28 browser suites, the deep tier's 12 checks green on the promotion PR, `python -m build` clean at 0.7.0. The soak — the gate on every promotion — was **red on half its runs until 2026-09-04**, and is exact now. **Downgraded 🟢→🟡 and back to 🟢 within the day, on the gate's reliability rather than the tests':** the SIL job produced two intermittent reds on PR diffs that could not touch them, both green on re-run — one of them a shutdown test that finishes in under 2 s locally and could not finish in 30 s on the runner. Every test passes; the *gate* reddens on unrelated work, and by this project's own standing rule that is a latent race, not noise. A gate you re-run is not green in the sense this criterion means, because it teaches the next reader to merge through red. **PR #143 reproduced BOTH and neither was a loose timeout** — the shutdown check read `proc.poll()` immediately after stdout EOF, which can answer `None` for a process the kernel has already reaped (3 of 6 runs under load, verdict reached in ~0.05 s, so the 30 s it blamed never elapsed); and every SIL client published `/state` before its SUBSCRIBE landed, against a config push that is **QoS 0 and not retained**, so losing that race deletes the message and no timeout is ever long enough. Neither bound was widened. Back to 🟢. **Downgraded 🟢→🟡 again on 2026-09-06 (evening), by this row’s own standard.** The rule stated three sentences up — *a gate you re-run is not green, because it teaches the next reader to merge through red* — applies to the day’s own events. `sim/test_liveliness.mjs` reddened **three PRs on diffs that could not reach it**: #202 (Python + YAML + Markdown) and, twice, **#201, the standing `dev → main` PR**, at `88.5px for 91px` and `86.8px for 89px`. Its `__bubbleAnchor()` readout compared `--leader` (**recorded**, from the last placed frame) against a head **re-projected live at call time**, so the two sides came from different frames and the error was one frame of head travel — in a file whose own header declares *“EVERY ASSERTION READS RECORDED STATE, NEVER A LIVE SAMPLE”*. PR #203 fixed it (single-instant stash; tolerance **2 → 0.2** on a measured 0.06 px residual; the face rule strengthened, not weakened) and reproduced the failure on **untouched `origin/dev`**, 5 of 65 under 2× CPU oversubscription. **The amber is for what #203 did not close**: the same slice measured that `chatting marker is cleared` and `paused hint goes away` **also fail on untouched `dev` under that load** — a `settle(4600)` sleeping on Node’s clock while the hold lapses on a page timer. That race is live on `dev` as this is written (`feat/holdwait` in flight), and `feat/suiteaudit` is sweeping the other 30 gating suites for more of the same family. Green returns when a gate reddens only for reasons about the product. **Re-scored 2026-09-07: still 🟡, and now for a measured reason rather than an open one.** The family closed **ten of twelve** instances (#203, #204, #206, #208, #209 plus the five earlier), and the two that remain are **filed with numbers, not suspicions**. `run_smoke.sh` — the script this program uses to answer *"does a real turn round-trip through the built backend?"* — was measured on one commit at four loads: **0 failures in 13 runs at load 3.3–75, and 2 in 10 above load 120.** That is a load-dependent probability, and the first reading of it (*"a threshold"*, from 8 passes then 2 failures) was **retracted the same day** when ten further runs came back clean, three of them inside the band the first reading had called failing. **The step was identified later the same day, on the sixth attempt, once the measuring wrapper stopped destroying the evidence it was counting.** It is the config-push wait — `sim/run_smoke.sh` line 241, a fixed `--timeout 20`. The captured log rules out the cause this project already knows: `subscriptions acknowledged` precedes `→ state`, so it is **not** the QoS-0-and-not-retained race PR #143 fixed. So the distinction that decides this row resolves as **both, entangled** — it is a fixed wait, but 20 s is not a tight budget, and what it is really measuring is a supervisor starved by ~150 spinning cores. **The defect is that the check cannot tell that from a wedged appliance**, and its message — *"no config pushed within timeout"* — reads unambiguously as the latter. The fix has a precedent in this repo: ask the supervisor whether it is alive when the wait expires and say which it was, exactly as PR #209 stopped `test_csp.mjs` comparing `"timeout"` against `"loaded"` and `"refused"` as though it were a third verdict. **That landed 2026-09-07 as PR #212** — the wait now asks the supervisor over HTTP, a different transport from the MQTT that went quiet, and reports **ALIVE** (a lost message or a starved process), **NOT ANSWERING** (wedged, gone, or starved past 3 s), or **NOT CHECKED** when it was given no URL. Eight tests; four of eight fail with the diagnosis reverted. **The row still stays 🟡, and the reason is narrower than it was:** #212 made the failure *legible*, it did not make it *stop*. The rate is unchanged — **0 failures in 21 runs at load 3.3–75, 3 in 19 above load 120** — and this criterion asks that a gate redden only for reasons about the product. A gate that reddens on starvation and now says so is a better gate and still not a green one. Green needs the failure gone or shown to be the product's; the next occurrence will finally carry the evidence to tell which. The other filed instance, `head-travel-threshold`, is the first this project **introduced while fixing this family** (block 4a, PR #203). Its repair was written and **reverted** over an unexplained motor stall; a later probe eliminated `setSpeech()` as the cause. Both are in `docs/architecture/backlog/`. **2026-09-08 — `head-travel-threshold` got a second attempt, and the attempt lost to the code it replaced.** The fixed four-frame wait was rewritten as a condition-bounded wait with the verdict split in two (*the runner was too slow* vs *the drive did not move her*). A prediction was committed **before** the comparison arm reported, stating that the original **must** fail at no more than 31 px because the rewrite runs a floor of four frames and so can never run fewer. It was falsified in one pair: rewrite **FAILED at load 66.74** (3/16 sweeps timed out, 31 px), untouched original **PASSED at load 78.85** — the harder load. Three attempts to find the mechanism in source produced only plausible stories, so the change was **reverted on those committed terms rather than tuned until green**, and the open question is now stated as a measurement (`docs/architecture/backlog/head-sweep-wait.md`): why does a loop running *at least* as many frames per sweep record *less* head travel? **Honest limits of that number: one pair, loads 12 apart, minutes apart, on a machine concurrently running unrelated suites whose load was not controlled and only noticed afterwards.** `n=1` falsifies a *must*; it does not establish a mean, and that claim is not made. The split verdict is the part that demonstrably worked and is filed to be added to the existing wait as diagnostics only. |

**Honest total: ~85 % — unchanged on 2026-09-08, and stated rather than skipped.** A day that reverted its own change, filed four briefs and falsified its own prediction produced no score movement, because none of it shipped capability: the reverted rewrite left `dev` where it started, and the grounding gate that would judge the one merged fix has still never run. Recording a flat score is the same discipline as recording a downgrade. Four green, two amber. This number has moved seven times in three days
(~80 → ~88 → ~82 → ~90 → ~85 → ~90 → ~85) and **that is the point, not a defect in the scoring**: four of
those moves were downgrades made after re-reading evidence already written down and finding it did not
support the claim. Each earlier one was earned back by fixing the thing rather than by re-describing it.
A score that only ratchets upward is a score being managed rather than measured.

The two ambers are different in kind, and collapsing them would be dishonest:

- **#4 is a ceiling, not work.** No physical Moxie has ever connected to this broker, so six green is
  unreachable from this room no matter what is built here.
- **#6 is work, and it is in flight.** A measured, reproducible race sits in a gating browser suite on
  `dev` as this is written — not a suspicion: it fails on **untouched `origin/dev`** under 2× CPU
  oversubscription. Today that family reddened the promotion PR itself twice.

Read ~85 % as *“of what this program can reach without a robot in the room”*, not as a fraction of a
finished product — and note that the previous total of “five green, one amber” was **stale in both
directions at once**: it had not been re-scored after #1 was restored to green, while #6 had drifted
below green unrecorded. Two errors cancelling is not the same as being right.
