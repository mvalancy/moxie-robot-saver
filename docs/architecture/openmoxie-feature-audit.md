...
> **Audit version 1 · baseline 2026-09-02 · status column refreshed 2026-09-04.** A feature-by-feature read of **OpenMoxie** (upstream `v0.8`, MIT)
...
> **2026-09-04**, and in [`orchestration-plan.md`](orchestration-plan.md)'s status log. Where a
...
>
> **Upstream re-check — checked 2026-09-04: still nothing new upstream, for the third consecutive day.**
...
> 🟡 **Launch-card QRs — open, and this bullet was *wrong* rather than merely stale until 2026-09-03.**
...
> **Where it stands 2026-09-04:** P0-a's *wire* half shipped (PR #119, above) and
> `volley.execution_actions` now reaches it (PR #121), so an `execute` can be **named**; the `ENABLE_QR`
> spelling defect, the `$eb_qr_value` → launch route against a closed allowlist (P0-b), and the printed
> sheet (P0-c) are all still owed.
...
| [`backlog/qr-launch-cards.md`](backlog/qr-launch-cards.md) | **§4.4 #9** (printable launch cards) — the row **re-scoped**: arming the runtime QR reader (`eb_enable_qr` as an `execute` action), routing `$eb_qr_value` into a real launch against a closed 24-id allowlist, *then* the sheet a parent prints | 🟢 **build-ready — spec'd 2026-09-03.** Written because the ranked row was **wrong, not stale**: it priced an **S** on a parser that only ever reads a brain's own reply text. **M**, three pieces. **P0-a's wire half shipped 2026-09-04** (PR #119 + #121); the `ENABLE_QR` spelling defect, P0-b (the `$eb_qr_value` route) and P0-c (the sheet) are owed. Honest ceiling: the end-to-end claim is unprovable on hardware — no physical robot has ever sent us a vision event |
...
## 4.4 The open backlog, re-ranked — 2026-09-04
...
### What this table got wrong, three times in twelve hours

This section is rewritten rather than edited because its *prose* — not just its statuses — was measurably
wrong three times between 2026-09-03 and 2026-09-04, and each error cost a run. They are kept here
...
`origin/dev` at `3c42423` on 2026-09-04.

### Shipped since the last refresh — corrections first, so nobody rebuilds them

| Landed | What it actually does | Where it is proved |
|---|---|---|
| **PR #121** | `act.<name>` is honoured: `volley.execution_actions` → `content_app.execution_actions_of` → `Reply.actions` → the wire. **Two of four** conformance rows green | `sim/tests/test_ext_act.py`; `test_ext.py::test_t1_t6_conformance_act` (G2/G3) — G5/G6 still `xfail(strict=True)` |
| **PR #119** | An `execute` action names the function it wants run — `function_id` / `function_args` / `action_args` | `sim/tests/test_actions_reach_the_robot.py` (written to pin the defect, then flipped) |
| **PR #116** | The virtual robot **acts on** `response_actions` instead of merely receiving them. DoD criterion 4's green was **not earned** before this | `sim/tests/test_actions_reach_the_robot.py`, `test_sim_client_parity.py`, `goldens/cloud_to_robot_actions.json` |
| **PR #113 + #118** | A child-safety bypass closed on **both** floors — hosted (`safety.js`) and local (`safety.py`) — where one U+00AD or U+2060 between letters defeated a hard block | see §4.2 BEYOND #2 |
| **PR #115 · #107 · #111** | `/api/*` hardening headers where `_headers` structurally cannot reach; a bounded FIFO admission queue instead of a refusal at the ceiling; four spend holes, one of them a **live IPv6 rate-limit bypass** (a /64 is 2^64 buckets) | see §4.3's `live-sim-demo.md` row |
| **PR #112 · #122 · #124** | The typed turn reaches the cloud brain and dead controls are disabled; the landing page stops accumulating draw work in a hidden tab; ambient self-talk no longer cuts off a live answer | same row |
| **PR #120** | **Nine browser suites had never executed in CI** — the harness scanned a developer-machine path, `skipper()` called `process.exit(0)`, and the job stayed green. A missing browser is now a CI failure | `.github/workflows/ci.yml`, `sim/browser_harness.mjs` |

> **PR #120 is the one to internalise.** It did not add coverage; it revealed that the repo's *best*
> guards — the peak-amplitude assertion that a silent clip cannot pass, and the `/api/chat` call count
> that proves a refused microphone spends nothing — had never run anywhere. That is the same lesson the
> unreachable-child-voice tombstone taught in PR #87 (*770 assertions that all read a file*), arriving a
> second time by a different door. **A green suite is evidence about the suite until you know it
> executed.**

### 🔒 In flight right now — do not assign

- **Accessibility of the hosted Sim** — being built in `feat/a11y`.
- **The browser half of client parity on `execute`** — being built in `feat/client-parity`.
  [`bridge.js`](../../sim/web/bridge.js):258 reads `entry.function` only, not `function_id` and no args,
  so an `execute` the SIL robot names renders as `(unnamed)` in the browser. Two clients that disagree is
  exactly what DoD criterion 4 forbids — which is why it is being fixed, and why nothing below ranks it.

### The ranking

| # | Open item | Outcome | Readiness | Why it ranks here |
|--:|---|:--:|:--:|---|
| — | **The unreachable child voice** (live-Sim assumption 22) | ① | 🟢 **SHIPPED 2026-09-03 — do not assign** | Kept for one refresh as a **correction**, because this row told two agents it was in flight long after it merged. It shipped as **PR #82** (`speakClipOnly` — a separate entry point rather than a flag, so the guarantee is *which function you called*: only a clip this site authored for that exact sentence, with no route to Piper, `speechSynthesis` or the tone generator, and a strict same-group lookup so `playClip`'s cross-bucket fallback cannot let a child line play in Moxie's voice) and was **corrected in PR #87**, which is the part worth keeping: #82's 770 assertions all read a *file* — Web Audio is stubbed in the node tests, so nothing had ever loaded the page — and driving the shipped `/sim.html` in a real Chromium found the feature half-landed, cutting *"…it's my birthd—"* at 2 237 ms of a 2 519 ms clip and clearing a *dropped* second line by about 700 ms of luck. **PR #96 did not touch it** (it is the broker-outage proof; the pairing in earlier notes is wrong). The general lesson is the one this table keeps relearning: a green suite that never loaded the artefact is not evidence about the artefact. **Nothing here is assignable** — this row is a tombstone and the next refresh should delete it |
| 1 | **Live-Sim P1** — exact counters, Turnstile, the TTS cache | ① | 🟢 **build-ready** | The headline goal is *a stranger opens the production domain and Moxie answers*, and P0 shipped with its own ceiling written down: the rate counter is an in-isolate map and nothing else — **per-isolate, and the Cache API leg named here until 2026-09-03 was never implemented** — so it is not a ceiling at all ([`live-sim-demo.md`](backlog/live-sim-demo.md) §4.6). Everything else on this list protects a robot in someone's house; this protects the one surface the public can reach and the one budget a stranger can spend. Brief: [`live-sim-demo.md`](backlog/live-sim-demo.md) §9 P1 — but note assumption 13 (**is KV or a Durable Object even available on this plan?**) is unverified, and the answer changes the design, so *check the dashboard before writing code* |
...
| 3 | **Production hardening for a robot that stays connected** (Fork A's list) | ① | 🟢 **SHIPPED 2026-09-03** *(P0 **and P1**; was 🟠 that morning)* | Done, and named so the claim is checkable: **the store** — `flock` on a per-record `.lock` sidecar behind a public `JsonStore.transaction()` in [`store.py`](../../mqtt/moxie_sdk/store.py), reached from all five `MemoryStore` read-modify-writes, plus the directory `fsync` after `os.replace` — proved by [`test_store_concurrency.py`](../../sim/tests/test_store_concurrency.py) (T1: two processes × 250 appends lose **0**, where `origin/dev` lost 500 of 1 000 every run). **The connection** — `connect_async` + `retry_first_connection=True` + `reconnect_delay_set(1, 60)`, a **checked CONNACK**, `on_disconnect`/`on_connect_fail`, one `_publish()` that reads `info.rc` at all eight sites, and `_on_event` registering an unknown device like `_on_state` always has — proved by [`test_connection_resilience.py`](../../sim/tests/test_connection_resilience.py) (S6 runs the real `run()` against a dead port and requires a *second* attempt, so it fails on the half-done fix too). Three real bugs closed: `_on_connect` logged *"broker connected"* for a refusal and subscribed anyway; the wakeup route answered `published: true` into a dead socket because its guard was `client is None` — PR #55's own bug, surviving in the one place that fix did not look; and `connect_async` alone is a no-op under `loop_forever`. 35 mutations, 0 missed ([`hardening_mutation_check.py`](../../sim/tools/hardening_mutation_check.py)). **P1 followed the same day**: `sim/run_soak.sh` (three profiles, every §5.3 bar printed pass-or-fail), the durable roster, the connection telemetry stream on the console's 📈 card, and the SIGTERM handler — 64 mutations, 0 missed, and the soak run for real (1 046 turns, 0 lost while up, reconnect p95 0.62 s, 0 lost updates across 4 contending processes). It also closed a **defect class** neither phase's plan predicted: the roster ghost and the never-cleared vision/STT latch are the same bug — a cached belief about the robot outliving the robot's state — and the 12th soak bar exists because eleven green ones sat on top of it. **The ceiling is unchanged and is the point:** no physical Moxie has ever been on our broker, no physical robot has ever sent us a vision event, six of the brief's assumptions still need one, and the lock timeout and the reconnect ceiling are still *chosen, not measured* — P1 built the instrument that would settle the first, and an instrument is not a measurement |
| 4 | **The behavior planner** (BEYOND #1) | ③ | 🟡 **P1 shipped 2026-09-03 — scored output everywhere, the performance only off the model path** (P2 open) | `moxie_sdk/performance.py` scores a line's **dialog act** (all 22) and stages a validated `Performance` that exactly one `render()` turns into markup, behind the same `make_markup` seam. C1–C7 landed for the *scored fields*: `mood`/`mood_intensity`/`dialog_act`/`emotion`/`signals` are filled on **every** published turn including streamed chunks (plumbed and empty since PR #17), and `POST /local/robots/{id}/preview` rehearses a line on the SIM before a child hears it. Deterministic, no model call, p95 0.25–0.56 ms, and **no first-audio regression** — re-measured as the real experiment, not a bench (`sim/tools/first_audio_ab.py`: 356.5 ms planner vs 357.9 ms floor over 20 controlled turns per arm). **The honest limit, found by integration and pinned by `test_the_model_path_performs_the_floors_markup`:** on the `llm` brain a real deployment runs, `LLMApp` authors its own markup and `_stage` honours it **verbatim by design**, so the markup a robot performs is the floor's `annotate` output **byte for byte**. `MOXIE_EXPRESSIVE=planner` changes the five scored fields, **not the performance** — the gaze, the per-clause mood and the staging never reach the wire on the model path (C6), and since only `LLMApp` streams, **no path carries planner markup on a streamed chunk at all**. Closing that is a design call on the turn loop, not an integration fix. **P2 — a learned/model-assisted scorer — is still open.** Brief: [`expressiveness.md`](backlog/expressiveness.md) §2 |
...
| 6 | **Content authoring, not just content distribution** (§3.3's authoring-UI row) | ②③ | 🟡 **P0 SHIPPED 2026-09-04 — a parent can write one; P1 (the paid *try*) and P2 open** *(was 🟢 build-ready that morning)* | **P0 is built: the 📦 card is now 📦 Content, with a ＋ New row, a per-item ✏️ and an editor whose default surface shows no JSON at all.** A parent writes a conversation (name, opening lines, a prompt box with four insert-chips) or a command (a list of phrases, compiled into the `pattern` for them), presses *Show what Moxie will be told* and reads the **resolved system prompt** — rung 1, and it costs nothing, because `POST /content/render` calls no brain and writes no byte. Save goes through `POST /content/item`, which validates exactly as an import does, snapshots into the same one undo slot and calls `reload_content()`, so the next turn uses it with no restart. The editor **refuses**: a schedule (§0 — the one kind that reaches the robot), any change to `code` or `extension` (both shown, both round-tripped byte-identical), any field outside `packs.FIELDS`, and a second tab's stale save (**409**). The only new safety code is the `validate_item` call `mark_edited` does not make, and [`sim/tools/authoring_mutation_check.py`](../../sim/tools/authoring_mutation_check.py) deletes it — 15 mutations, 15 caught. **What P0 deliberately did not build:** `POST /content/try` and every brain call with it, extension authoring, schedule authoring, a writable raw surface, deletion and a second author. Packs made content *shippable*; nothing made it *writable*. A parent can install a stranger's conversation and diff it against their own, and still cannot compose one without editing JSON — and upstream's browser chat harness (`/hive/interact`) remains a genuinely better authoring loop than ours. New to this ranking, because packs shipping is what exposed it. **The brief now exists and removed all four decisions it owed** ([`content-authoring.md`](backlog/content-authoring.md), §11 checks itself against them): authoring lives in the **parent console** as a second verb on the 📦 card — the only surface that already holds the content store, the validation path and the rehearsal hook — and the write seam was pre-declared in code, since [`packs.mark_edited`](../../mqtt/moxie_sdk/content/packs.py)'s docstring names *"a future 📦 edit button"* as its consumer. A parent gets a form, not a JSON box: three progressively-disclosed surfaces, a closed chip list that emits **only** the two template forms the dependency-free fallback also renders (so a guided prompt is portable by construction), phrases compiled into a `pattern` rather than a regex box, and `code`/`extension`/`schedule` refused outright. The loop is five rungs and **exactly one spends a gateway call**: typing, the resolved-system-prompt panel and rehearsing an opener through the existing `POST /preview` are free; a *try* is one brain call per click behind `MOXIE_AUTHOR_TRY_BUDGET` with `remaining` on screen; a **command** try costs nothing at all. Safety is six gates that are all existing functions — the one piece of new code is a `validate_item` call, because `mark_edited` normalizes and does not validate. **P0 S/M, P1 M, P2 L.** Honest ceiling: **three of twelve assumptions need a real parent** (A9–A11) and are untestable by us; A9 is the one that would flip the location choice |
...
| 9 | **Printable launch-card QRs** (`GO<launch:MOD>`) | ② | 🟢 **build-ready — RE-SCOPED 2026-09-03; it is not an S** | **This row was wrong, not stale**, and it is kept rather than deleted so the correction is checkable. It read *"the action-tag parser already understands `<launch:MOD[:CID]>`; what is missing is a sheet a parent prints"* — a real mechanism pointed the wrong way: `parse_action_tags` is only ever called on **a brain's own reply text** ([`llm_app.py`](../../mqtt/moxie_sdk/apps/llm_app.py):407, :442, :480 · [`content_app.py`](../../mqtt/moxie_sdk/content/content_app.py):182, :394 · [`webhook_app.py`](../../mqtt/moxie_sdk/apps/webhook_app.py):59), the **cloud→robot** direction, while a scanned card arrives **robot→cloud** as `input_vars['$eb_qr_value']` on a vision turn that [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py)`::_on_vision_turn`:2669-2690 answers with a greeting or `NOREPLY_ACK` and never consults. Print the sheet alone and the cards do nothing. Two facts set the real scope. **(1) The setup app's grammar is provably closed** ([`qr-commands.md`](../reverse-engineering/protocol/qr-commands.md):87-100 — three `QRData.ParseFromString` branches, **four** `debug.command` literals, a literal `else → State.QRDiagnostic`, no fifth `bo-wifi` handler, zero `QRCommand` references in `Assembly-CSharp`), so a card **cannot** ride the scanner that reads the pairing code. **(2) The *runtime* reader is the real path** — it raises `eb-qr-event`, whose string reaches us as the `speech` of a `RemoteChatRequest` ([`vision.md`](vision.md):73-74; [`presence.py`](../../mqtt/moxie_sdk/presence.py):18, :65, :75) — and [`test_presence_runtime.py`](../../sim/tests/test_presence_runtime.py):125-128 already drives one carrying the literal `GO<launch:DM>`, asserting only that we *remember* the string. So the slice is **three** pieces, not one — **arm** (`eb_enable_qr` as an `execute` action: [`wire.py`](../../mqtt/moxie_sdk/wire.py):82-85 drops `Action.function`/`args`, and `ActionType.ENABLE_QR` ([`types.py`](../../mqtt/moxie_sdk/types.py):61) is a verb the recovered `ActionID` enum does not name), **route**, **print** — priced **M**. Brief: [`backlog/qr-launch-cards.md`](backlog/qr-launch-cards.md). **Ceiling, inherited from BEYOND #9 and unmoved:** no physical robot has ever sent us a vision event, so the end-to-end claim is **unprovable on hardware** whatever we build; what is provable is the decoder, the runtime's reply and the SIL/browser clients |
| 10 | **One identity, one guided first run** (BEYOND #10) | ① | 🟠 **needs-a-spec** | The parent app keeps real child records (`POST /api/children`) while the supervisor builds one `ChildProfile` from an env var (`mqtt/run.py`:54 — this row said `:35` until 2026-09-03, which is a different function) — and the hosted edge tier is now a third, deliberately childless surface. Reconciling three registries is a data-model decision before it is code |
| 1 | **Content authoring, not just content distribution** | ②③ | 🟢 **build-ready** | ⭐ **The most valuable next slice — see below.** Packs made content *shippable* and nothing made it *writable*: a parent can install a stranger's conversation and diff it against their own, and still cannot compose one without editing JSON. The brief removed all four decisions it owed ([`content-authoring.md`](backlog/content-authoring.md) §11 checks itself against them) — authoring is a second verb on the console's 📦 card, the only surface already holding the content store, the validation path *and* the rehearsal hook; the write seam was pre-declared, since [`packs.mark_edited`](../../mqtt/moxie_sdk/content/packs.py)'s docstring names *"a future 📦 edit button"* as its consumer. A form, not a JSON box; a closed chip list emitting only the two template forms the dependency-free fallback also renders; `code`/`extension`/`schedule` refused outright. Exactly one of five rungs spends a gateway call. **P0 S/M.** Ceiling: A9–A11 need a real parent, and A9 would flip the location choice |
| 2 | **QR launch cards — P0-b and P0-c, plus the `ENABLE_QR` spelling** | ② | 🟢 **build-ready** | The wire half landed under it (#119, #121), so what is left is the part that was always the actual work. **P0-b** routes `input_vars['$eb_qr_value']` into a launch against a **closed 24-id allowlist** — the first time anything in our cloud *acts* on a perception event rather than noticing it. **P0-c** is the sheet a parent prints. And a defect must be fixed rather than inherited: `ActionType.ENABLE_QR = "enable_qr"` is **not a name in the recovered `ActionID` enum** (`launch`, `launch_if_confirmed`, `exit_module`, `request_next`, `abort_module`, `execute`, `sleep`, `tangent`) and must serialise as `execute` + `function_id: "eb_enable_qr"`. That rename is now **pinned by a test named for it**, so making it turns a test red and has to say why. Do **not** touch `ActionType.EXIT` in the same slice (§7 R3). Brief: [`qr-launch-cards.md`](backlog/qr-launch-cards.md) |
| 3 | **A host for `subscribe`** — the third conformance row | ③ | 🟢 **build-ready** | The smallest well-defined win on this page and the one that makes the sandbox *perceive*. `Volley.subscriptions` exists; `wire.build_chat_response(subscribe_events=…)` exists; **nothing joins them**, because the supervisor fills `EventSubscription` from its own vision bookkeeping (`moxie_runtime.py::_publish_chat`). Joining them — pack-requested subscriptions merged into the supervisor's set, not replacing it — turns G6 green and takes the golden set to **3 of 4**. It also composes with #2: `eb-qr-event` is the event a QR card arrives on. The `xfail(strict=True)` marker means the day it works, the suite fails until someone removes the marker |
| 4 | **The parent-facing capability grant flow** | ③ | 🟠 **needs-a-spec** | The honest ceiling under #121, and it is not small: `act` is now *grantable* and **nothing in the shipped appliance grants it**. `mqtt/config.py`:495 builds `ContentApp` with no `ext_grants`, and `SHIPPED_EXTRA_GRANTS` deliberately excludes every `act.<name>` — so today an `act` is reachable from a test and from no parent. Deciding *how a parent grants a capability* (the console card, the pack-review tick, what a re-grant means when a pack updates and asks for more) is a design argument before it is code, and `ext.grant_list()` + the capability-escalation rule already exist to be rendered by it |
| 5 | **Live-Sim P1 — and it is three items, not one** | ① | 🟢 / ⛔ **split** | This row said 🟢 **build-ready** as a whole until today, and two thirds of it is not. **🟢 The TTS response cache and the nonce CSP** need nothing from anyone and are the buildable half — `functions/api/speech.js` consults no cache today (grep: no `caches` reference in the file). **⛔ The exact counters** are blocked on [`live-sim-demo.md`](backlog/live-sim-demo.md) §10 **assumption 13** — *is KV or a Durable Object even available on this account and plan?* — which is a dashboard question with an owner's answer, and the answer decides the design. §4.6's own note is explicit that the Cache API tier is *deliberately not* added until 13 is answered. **⛔ Turnstile** needs a sitekey and secret minted on the account: also an owner action. So the ranked, assignable work here is the cache; the rest is an owner's five minutes |
| 6 | **Insights that mean something** (BEYOND #5's second half) | ①③ | 🟠 **needs-a-spec** | The durable half shipped (PR #55) and the *interesting* half did not: no sessions, no activity mix, no mood trend, no time-of-day pattern, no *"what did we talk about this week"*. 🟠 because the blocker is a **vocabulary**, not an implementation — `Packet.event_name` is a free string and our corpus recovers no module-scoped event, so the slice is a choice between deriving them from `mentor_behaviors` + conversation history and *emitting* our own named events from the runtime. That choice wants a spec |
| 7 | **The behavior planner P2** (BEYOND #1) | ③ | 🟠 **needs-a-spec** | P1 shipped 2026-09-03: the five scored fields are on every published turn. The limit is pinned by its own test — on the `llm` brain a real deployment runs, `LLMApp` authors its own markup and `_stage` honours it **verbatim by design**, so the gaze, the per-clause mood and the staging never reach the wire on the model path, and since only `LLMApp` streams, no path carries planner markup on a streamed chunk at all. Closing that is a design call on the **turn loop**, not an integration fix — which is what makes it 🟠 rather than build-ready |
| 8 | **`TTSMark[]` visemes** (the rest of BEYOND #8) | ①③ | 🟠 **needs-a-spec** | Re-verified 2026-09-04: `marks` is plumbed through [`moxie_sdk/tts.py`](../../mqtt/moxie_sdk/tts.py):370-402, read back at :402, echoed by `sim/virtual_moxie.py`:374 — and **populated by nothing but tests**. So the SIM's mouth is animated by amplitude rather than phonemes. 🟠 because *can Piper emit the alignment we need* is an unanswered question, and the hosted path now encodes its own WAV, which changes where the marks would come from |
| 9 | **One identity, one guided first run** (BEYOND #10) | ① | 🟠 **needs-a-spec** | The parent app keeps real child records (`POST /api/children`); the supervisor builds one `ChildProfile` from an env var (`mqtt/run.py`:54); the hosted edge tier is a **third**, deliberately childless surface (`functions/api/` — 4 routes + 7 `_lib` modules). Reconciling three registries is a data-model decision before it is code |
| 10 | **Broker auth P1/P2** — device credentials, then a refused spoof | ①② | ⛔ **blocked on A1–A4** | P0 confines a client to its own subtree, so this is no longer *"anyone can read another child's `child_pii`"* — it is *"the broker still cannot tell **which** robot a client id belongs to"*: containment, not identity. It is last **only** because it cannot start. [`security-broker-auth.md`](backlog/security-broker-auth.md) §0.4's A1–A4 each need a **physical robot**. Assigning it produces a rewritten brief, not a merge |
...
it); **C4** — the owed *verification* that STT re-subscribes after a robot sleep/wake. Both are specified
on [`backlog/community-signals.md`](backlog/community-signals.md); neither needs a decision. A build agent
finishing early should take one.

### ⭐ The single most valuable next slice — content authoring P0

Not because it is the largest, and not because it is the newest. Because of what is left on the list once
the honest verdicts are applied: **#5 is two-thirds an owner's dashboard, #10 needs a robot, and #4, #6,
#7, #8 and #9 all owe a decision before they owe code.** That leaves three genuinely build-ready items,
and of those, #2 and #3 change what the *appliance* can do internally while #1 changes what a **person**
can do with it.

It is also the item this project's own progress created. Packs (PR #51/#78) made content shippable and
in doing so made *un*-writability the binding constraint: we can distribute a conversation nobody here
can author without a text editor. Upstream's browser chat harness (`/hive/interact`) remains a genuinely
better authoring loop than ours, which is the last place on this page where that sentence is still true —
and outcome ② is measured by exactly that.

**Start at P0 (S/M), not P1.** The brief's own §11 audits itself against the four decisions it removed,
and the one piece of genuinely new code in P0 is a `validate_item` call, because `mark_edited` normalizes
and does not validate. Everything else is an existing function wired to a form.

> **Runner-up, and why it lost.** #2 (the launch cards) is more fun, more visible and equally ready — but
> its browser half is being touched *right now* in `feat/client-parity`, and its end-to-end claim is
> unprovable without a robot. #1 collides with nothing and can be finished.
...
2. **No human has ever recorded through the hosted mic.**
...
**Verified nothing, three times.** There is no upstream item on this list because there is nothing new
upstream: all three source repos were re-read at their live remote heads on 2026-09-04, the whole 40-fork
network was checked for a push after 2026-09-03 (zero), and the newest activity anywhere in upstream's
tracker is 2026-08-29 — *before* the previous sweep. The evidence is in
[§2.6](#26-upstream-re-check-2026-09-04-nothing-moved-and-that-is-the-whole-finding). What remains is
ours to build.
