...
> **Audit version 1 · baseline 2026-09-02 · status column refreshed 2026-09-05 · every `ADOPT`/`BEYOND`/`none` claim re-verified against the code 2026-09-06.** A feature-by-feature read of **OpenMoxie** (upstream `v0.8`, MIT)
...
|---|---|---|
| **[Noonster77/openmoxie](https://github.com/Noonster77/openmoxie)** — "OpenMoxie Family Edition" | 2026-08-31 (active — verified 2026-09-06) | Local-first AI (LM Studio + faster-whisper), a rebuilt family-facing UI, speaker-scoped memory, transcripts, safety flags + parent review, 70 tests |
...
> 🩹 **2026-09-06 — the freeze was not enough, and the fix is in §3 itself now.**
> On 2026-09-06 the orchestrator twice began briefing an agent to build something already shipped, and
> **two of the misleading cells were §3 rows** — §3.2 *"nothing parses tags out of model text"* and §3.3
> content versioning *"none"*. This table existed, said both were closed, and was **read past**: a reader
> who lands on a verdict cell reading **ADOPT** does not scroll up to find out that §3 is a museum.
> A warning that depends on being read in order is not a control. So every **ADOPT** / **BEYOND** verdict
> cell in §3.1–§3.5 now carries its own inline 🩹 verdict, re-measured against the code on 2026-09-06,
> citing `file:line` in **both** directions — a "shipped" and a "still open" are held to the same standard
> of proof. The *"Us today"* prose is still untouched, so the reasoning is still auditable; what changed is
> that a stale claim can no longer be read without its correction attached. This table stays as the index.
> **Sixteen of twenty-one §3 verdict cells re-measured this way were stale.**
...
| **Launch-card QRs** (`GO<launch:MOD>`) | `site/data/qr/extract.py` → 24 printable PNGs | none | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — three of four pieces SHIPPED.** The *decoder* is [`mqtt/moxie_sdk/launch_cards.py`](../../mqtt/moxie_sdk/launch_cards.py) — `decode` at :186 runs `parse_action_tags` over the scanned payload against a **derived** allowlist (`LAUNCHABLE_MODULE_IDS`, :41-46), and its one caller is `moxie_runtime.py::_on_vision_turn`; the browser twin is `sim/web/qr.js::encodeCard`, pinned byte-for-byte by [`sim/test_qr.mjs`](../../sim/test_qr.mjs):124-259. **What is genuinely still open is only the paper:** nothing generates a printable sheet, and [`launch_cards.py`](../../mqtt/moxie_sdk/launch_cards.py):139 names the sheet generator *"(unbuilt)"* in place. §4.4 #7. |
...
| Robot identity / JWT | `RobotCredentials` mints RS256 JWTs, can ADB-pull `uuid.txt` + `RS256.key` — but only to *impersonate* a robot: their broker is `allow_anonymous true` and never verifies one | broker is anonymous; we don't verify JWTs either (deferred, §3b). The pairing gate is **service refusal, not authentication** — an unpermitted device still connects, and a spoofed `d_<uuid>` is served as that robot | **ADOPT** (M) — 🔵 **brief ready**: [`backlog/security-broker-auth.md`](backlog/security-broker-auth.md) phases it P0 (a `%c` ACL + a supervisor credential, shippable against an unmodified robot) → P1 (the broker asks the supervisor to verify the RS256 JWT against an enrolled public key) → P2 (a spoofed id refused at CONNECT). *"Parity — both punt"* was true and useless; repriced. 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — half of this cell is stale, half is exactly true.** *Stale:* "broker is anonymous" — [`mqtt/broker/mosquitto.conf`](../../mqtt/broker/mosquitto.conf):10 sets `per_listener_settings true` and two ACL files (`mqtt/broker/acl`, `mqtt/broker/acl-robot`) confine each client to its own `%c` subtree (P0). *Exactly true:* "we don't verify JWTs either" — no RS256 verification exists anywhere in `mqtt/`; [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):2339 says so in place, and [`broker_acl.py`](../../mqtt/moxie_sdk/broker_acl.py):5 records that the robot listener carries no `password_file`. **Still open: P1 + P2**, blocked on A1-A4. |
...
| OTA push (801→803 lever) | notes only in `doc/RemoteModuleAPI.md` (`ota_update`, `_PROVIDE_HTTP_TOKENS`, static `url` file) | `ota_update`/`forbid_otaver` specified in [`config-and-telemetry-contract.md`](config-and-telemetry-contract.md), **not built** | **ADOPT** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — STILL OPEN, and it is the cleanest remaining "not built" on this page.** `grep -rn "ota_update\|forbid_otaver" mqtt/ server/ --include=*.py` returns **zero**: the keys are specified in [`config-and-telemetry-contract.md`](config-and-telemetry-contract.md) and no code reads or writes them. *What is missing, specifically:* no config key emitted by `cloud_config.py`, no HTTP token provider, no `url` file, no console control — and the whole row inherits the hardware ceiling (§4.4 ⛔). |
...
| **LLM response tags** (`<exit>`, `<launch:MOD:CID>`, `<sleep>`) | `Volley.ingest_action_tags()` — parses the model's own text into real actions and strips them | we have `Reply.actions`, but nothing parses tags out of model text | **ADOPT** (S) — very cheap model agency — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED; the claim "nothing parses tags out of model text" is FALSE and has been since PR #6.** [`mqtt/moxie_sdk/actions.py`](../../mqtt/moxie_sdk/actions.py):112 is `parse_action_tags` and :103 is `tidy_spoken_text`; the grammar (`<exit>`, `<sleep>`, `<launch:MOD[:CID]>`, `<launch_if_confirmed:MOD[:CID]>`) is credited to OpenMoxie in the module header. It is reachable from **three** real call paths, not one: [`apps/llm_app.py`](../../mqtt/moxie_sdk/apps/llm_app.py):407 (per streamed sentence), :442, :480; [`content/content_app.py`](../../mqtt/moxie_sdk/content/content_app.py):180 and :414; [`apps/webhook_app.py`](../../mqtt/moxie_sdk/apps/webhook_app.py):59. The parsed actions leave on the wire at [`wire.py`](../../mqtt/moxie_sdk/wire.py):157 (`resp["response_actions"]`). **This exact cell mis-briefed the orchestrator on 2026-09-06** — it is why this refresh exists. |
...
| Puppet / telehealth | full: `moxie_mode:"TELEHEALTH"`, `PLAY_OUTPUT`/`INTERRUPT`, mood+intensity picker, live state poll | `MoxieMode.TELEHEALTH` exists in `cloud_config.py`; **no command path, no UI** | **ADOPT** (M) — 🔵 **🟢 **built** (2026-09-02, `feat/telehealth`)**: [`backlog/telehealth.md`](backlog/telehealth.md) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED; "no command path, no UI" is false on both halves.** *Command path:* [`mqtt/moxie_sdk/telehealth.py`](../../mqtt/moxie_sdk/telehealth.py) (283 lines) — `telehealth_topic` :130, `build_telehealth_command` :209, `parse_telehealth_event` :248, mood/intensity validation :144-192. *UI:* the 🎭 "Be Moxie" card at [`server/static/index.html`](../../server/static/index.html):237, driven from [`server/static/app.js`](../../server/static/app.js):794-841 (session start/stop, live state). |
...
|---|---|---|---|
| Content model | `SinglePromptChat` rows, editable in Django admin, multi-CID via `\|` | `content/module.py` dataclasses loaded from JSON files; `content_modules/starter.json` has **1 conversation + 1 global** | **ADOPT** (see below) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — the count moved and a second module shipped.** Measured by parsing the files: `mqtt/content_modules/starter.json` = **1 conversation + 2 globals** (the second is the sandboxed-extension conformance activity G1), and `mqtt/content_modules/memory_chat.json` = **2 conversations**. The row's *verdict* is superseded by the authoring UI and pack rows immediately below. |
| **Content authoring UI** | dashboard list + Django admin + the `interact` browser harness | none — edit JSON by hand | **ADOPT** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (P0, PR #128); "edit JSON by hand" is false.** The ✍️ editor is [`server/static/index.html`](../../server/static/index.html):404, over five supervisor routes in [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py): `/content/export` :1007, `/content/undo` :1163, `/content/review` :1166, `/content/render` :1171, `/content/item` :1177. **What is still open is the P1 rehearsal rung, `POST /content/try`** — deliberately absent, and the appliance says so at [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):4602, with [`sim/tests/test_content_authoring.py`](../../sim/tests/test_content_authoring.py):567/:572/:623 asserting its absence on all three surfaces so P0 cannot quietly grow a brain call. §4.4 #6. |
| **Content packs: import/export + review** | `ExportDataView`/`export_data`, `upload_import_data`/`import_data`, with per-record `New / Upgrade from vN / Replace vN` | none | **ADOPT** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (PR #51, hardened #78); "none" is false.** [`mqtt/moxie_sdk/content/packs.py`](../../mqtt/moxie_sdk/content/packs.py) is **1 145 lines** (this page has quoted 942 since the baseline — the file grew). Review states at :150-157 are a **2×2**, not upstream's one integer: `UPGRADE` / `CONFLICT` / `DOWNGRADE` over `source_version` × `local_rev`. |
| **Content versioning** | `source_version` int on 3 models; `init_data` upgrades only when newer | none | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED; "none" is false.** `source_version` appears in **6 files** under `mqtt/` (`grep -rl`): [`content/module.py`](../../mqtt/moxie_sdk/content/module.py):113 (the field, with `_source_version` parsing at :125), [`content/packs.py`](../../mqtt/moxie_sdk/content/packs.py):150-157, `supervisor/moxie_runtime.py`, `content_modules/starter.json`, `content_modules/memory_chat.json`, `content_modules/README.md`. **Beyond upstream:** a `local_rev` digest means an upstream re-import reports `CONFLICT` rather than clobbering a parent's edit. |
| Globals — regex + actions | 4 action types (`RESPONSE`/`LAUNCH`/`CONFIRM_LAUNCH`/`METHOD`), `sort_key` ordering, lowercase matching | `Global` regex + entity groups + **registered handlers only**; no LAUNCH/CONFIRM_LAUNCH shorthand | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — STILL OPEN as written, and deliberately superseded in shape.** `grep -rn "CONFIRM_LAUNCH\|confirm_launch" mqtt/moxie_sdk/` returns **zero** — there is no shorthand action type. [`content/module.py`](../../mqtt/moxie_sdk/content/module.py):105-114 shows what a `Global` carries instead: `action:int`, `code` (never executed), and `extension:dict` — the sandboxed program of the row below. *What is specifically missing:* the four-constant shorthand and its `sort_key` ordering; what we built instead is a general evaluator. Not build-ready as a port — it is a design question about whether the shorthand is still wanted now `extension` exists. |
| Globals — stored Python (`METHOD`) | `exec` + 10 s `ThreadPoolExecutor` timeout | deliberately deferred (sandboxing) — `content_app.py` says so in its docstring | **BEYOND** (L) — see §4.2 — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (BEYOND #6 P0 2026-09-03, P1 `act` #121 and `subscribe` 2026-09-05); "deferred" is stale.** [`mqtt/moxie_sdk/content/ext.py`](../../mqtt/moxie_sdk/content/ext.py) is the capability-scoped evaluator; `P1_CAPABILITIES` at :230 is now down to `{brain, schedule.request}`. `code` remains, by design, **never** executed — that half of the cell is still exactly true. |
| **Schedule serving** | `RobotData.get_schedule` + `scheduler.expand_schedule` — generative day plan, category-variety RANSAC, chat interleaving, FTUE pruning; 3 seed schedules | `_on_activity` answers `query:"schedule"` with `{}` **and with the wrong shape** (generic `result` key; the robot expects `schedule` / `mentor_behaviors` and a `request_id` echo) | **ADOPT — the single biggest functional gap** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (PR #5 shape, PR #7 plan); this is no longer any kind of gap.** [`mqtt/moxie_sdk/schedule.py`](../../mqtt/moxie_sdk/schedule.py) is **1 016 lines**, and [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):2507 dispatches `schedule` / `mentor_behaviors` / `license` by name from `client-service-activity-log` queries (:2477). |
| **`mentor_behaviors` history** | `MentorBehavior` model, ingest + serve, indexed `(device, timestamp)` | returns `[]` | **ADOPT** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (PR #7); "returns `[]`" is false.** [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):4860 `mentor_behaviors()` reads the durable collection named at :77 (`robots/<id>/mentor_behaviors.json`, capped at `MAX_MENTOR_BEHAVIORS = 500`, :72); `ingest_mentor_behavior` :4870 writes it, policy-gated, and :4937 feeds it to the day planner. |
| Native-module launching (~23 `RECOMMENDABLE_MODULES` + DM) | `content/data.py` lists them all with categories | listed in our docs; **no code schedules them** | **ADOPT** (rides on the schedule work) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED with ADOPT #1; "no code schedules them" is false.** [`schedule.py`](../../mqtt/moxie_sdk/schedule.py):124 is `ONBOARD_MODULES`, scored by `CATEGORY_ENERGY`, `TIME_FIT`, `TIME_BUCKETS` and `FTUE_COMPLETION_COUNTS` in the same file, and named for a parent by `MODULE_LABELS`. |
...
| Execution actions (`eb_timer_request`, `eb_enable_qr`…) | `Volley.add_execution_action` → real `response_actions` | `Volley.add_execution_action` captures them; **not plumbed onto the wire** (plan §Known gaps) | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (PR #119 wire, PR #121 the producer); "not plumbed onto the wire" is false.** [`wire.py`](../../mqtt/moxie_sdk/wire.py):28 `encode_action` carries `function_id` (proto field 7) plus `function_args` (8) or `action_args` (10) by argument type; [`content_app.py`](../../mqtt/moxie_sdk/content/content_app.py):816 `execution_actions_of` is the producer, called at :184 and :419. |

### 3.4 Fleet, config, console

|---|---|---|---|
| Config model | **two-level deep-merge**: `HiveConfiguration.common_config/settings` (fleet) ⊕ `MoxieDevice.robot_config/settings` (per-robot) | one flat `build_robot_cloud_config()` + per-device `_config_overrides`; no fleet-wide layer | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED; "no fleet-wide layer" is false.** [`cloud_config.py`](../../mqtt/moxie_sdk/cloud_config.py):404 is `merge_config_layers`, and `defaults ⊕ fleet ⊕ per-robot` is the layering the brain picker and the look card both ride on. |
...
| **Progress / missions editor** | complete / forget / reset by writing `MentorBehavior` rows | nothing | **ADOPT** (M, after MBH) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — STILL OPEN, and it is HALF-BUILT — name the half.** *Built:* the records are durable and **readable** ([`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):4860) and **erasable in bulk**, because `ACTIVITY_COLLECTIONS` (:3000-3022) names `MENTOR_BEHAVIORS_COLLECTION` beside the packet ring and the daily roll-up, so one `erase_telemetry` clears all three. *Missing, specifically:* **a per-mission write path** — nothing outside `ingest_mentor_behavior` (:4870, the robot's own report) ever creates or removes a single row, so a parent cannot mark one activity complete, forget one, or reset one — **and no console UI at all** (`grep -n mentor server/static/*` returns nothing). Build-ready only after the write path is designed. |
| Wake command | `send_wakeup_to_bot` → `commands/wakeup` | `POST /api/robots/{rid}/wakeup` exists on the **REST** side; the MQTT `wakeup` command is not sent | **ADOPT** (S) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — SHIPPED (PR #55); "the MQTT `wakeup` command is not sent" is false.** [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):2558 defines `WAKEUP_COMMAND` and :2592-2593 publish `{"command":"wakeup"}` on `/devices/{id}/commands/wakeup`. **The ceiling stands and is not a gap:** our corpus establishes no acknowledgement, so a published `wakeup` is fire-and-forget (§4.4 ⛔). |
| **Telemetry / insights** | `MoxieLogs` model exists but is never written; no charts, no sessions | `Packet` build/parse/ingest + `LoggingPolicy` gate + `telemetry_count`; **console view still missing** (plan's own "most valuable next slice") | **HAVE** (model) / **BEYOND** (the view) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — the VIEW SHIPPED; "console view still missing" is false.** The 📈 Insights card is [`server/static/app.js`](../../server/static/app.js):255 (`weekBars` :267, `refreshInsights` :330): a zero-filled seven-day bar row over `telemetry_daily`, per-event counts, the event log, the real retention window, an honest `persisted:false` branch under `LoggingPolicy.NO_DATA`, and a two-click erase (:394). **What is still open is the row's interesting half** — no sessions, no activity mix, no mood trend, no time-of-day pattern (`mood` appears in `app.js` only in the telehealth picker). BEYOND #5 / §4.4 #8. |
| Multi-robot | full per-robot rows, schedules, config, persist, MBH | per-`device_id` everywhere in the runtime + `/local/fleet`; but **one `ChildProfile` for all robots** (`MoxieRuntime(child=…)`) and a single-robot console card | **ADOPT** (M) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — STILL OPEN, re-measured, and both halves confirmed.** One profile: [`mqtt/run.py`](../../mqtt/run.py):**54** builds `ChildProfile(nickname=config.CHILD_NICKNAME)` from an environment variable, and `moxie_runtime.py`:161 is `self.child = child or ChildProfile()`. *(This page and the plan both still cite `run.py:35`; the line moved on 2026-09-06 and the gap did not — cite **:54**.)* Single-robot card: [`server/static/app.js`](../../server/static/app.js):116 and :133 both render `robots[0]`. *What is specifically missing:* one device/child registry shared with the parent-app server's `POST /api/children`, and a robot selector on the console. §4.4 #11. |
...
|---|---|---|---|
| Storage | Django ORM + SQLite, 8 models, 16 migrations | `server/` = SQLite, 8 tables, no ORM. **`mqtt/` has no database at all** — memory + JSON files | **ADOPT** (M) — the robot cloud needs durable content/schedule/progress storage — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — STILL OPEN *as a database*, and badly understated as a description.** "No database" is literally true — there is no SQL anywhere in `mqtt/`. "Memory + JSON files" is not: [`mqtt/moxie_sdk/store.py`](../../mqtt/moxie_sdk/store.py) is **1 192 lines** of durable per-robot store with atomic `os.replace` writes, an `fsync` of the *directory*, a thread lock, and a cross-process advisory-`flock` `transaction()`, carrying 16 collections. Its own header (:13-16) points at this very row and calls itself *"a stepping stone, not a database"*, with the read/write/append/delete API deliberately narrow so SQLite can replace it without touching a caller. *What is specifically missing:* nothing is blocked on it today — this is a re-implementation, not a feature, and it should be re-priced before anyone is briefed on it. |
| **Prebuilt images / one-file install** | multi-arch images on Docker Hub; documented install is *download `docker-compose.yml`, run two commands* | root `docker-compose.yml` (one stack, clone+build) **and** a self-contained `docker-compose.images.yml` that pulls multi-arch GHCR images the release workflow pushes on every `v*` tag — *download one file, run one command* | **HAVE** (published at v0.6.0) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — the version moved:** `git tag --list` tops out at **`v0.7.0`**, not `v0.6.0`, and `.github/workflows/release.yml` is the workflow that pushes on a `v*` tag. Verdict unchanged. |
| Tests | `site/hive/tests.py` is the 3-line Django stub — **zero tests** | 37 test files (`sim/tests/*.py`, `sim/test_*.mjs`, `tools/robot-toolkit/test_*.py`) | **HAVE** (far ahead) — 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — **158** test files, counted 2026-09-06:** 112 `sim/tests/test_*.py` + 34 `sim/test_*.mjs` + 12 `tools/robot-toolkit/test_*.py`. (37 at the baseline, 105 on 2026-09-03.) Verdict unchanged. |
...
| # | Adopt | Why (one line) | Their file | Effort | Status — live, refreshed 2026-09-05, re-verified 2026-09-06 |
|--:|---|---|---|:--:|---|
...
| # | Go beyond | Why (one line) | Effort | Status — live, refreshed 2026-09-05, re-verified 2026-09-06 |
|--:|---|---|:--:|---|
| 1 | **A behavior *planner*, not a markup regexer** | `automarkup` maps words to tags. The 10× version scores each line for mood, dialog-act, gesture, gaze, screen icon and SFX from the recovered vocabularies, validates every asset reference before it ships, and previews the result on the 3D SIM — so authors *see* the performance before a child does. | **L** | 🔵 **open — spec'd.** Contract-level spec: [`backlog/expressiveness.md`](backlog/expressiveness.md) §2 (P0 floor → P1 planner → P2 model-assisted) 🩹 **RE-VERIFIED AGAINST THE CODE 2026-09-06 — P1 SHIPPED and this 🔵 was stale.** The planner exists and is the **default**: [`mqtt/moxie_sdk/performance.py`](../../mqtt/moxie_sdk/performance.py) is **839 lines** — `plan()` scores a line's `RemoteDialog.DialogAct` into a validated `Performance` of `Beat`s and `render()` is the only function that mints a mark — and [`mqtt/supervisor/markup.py`](../../mqtt/supervisor/markup.py):7 names `planner` as the default path, falling back to `automarkup.annotate` on `None` *and* on any exception. `types.py`:140 carries the staged `Performance` behind `markup`. §4.3's brief row and §4.4 #10 already said P1 had landed; **this cell was the one that did not, and it is the cell an agent scanning §4.2 for open work would read.** **Still open: P2**, the model-assisted half — §4.4 #10, 🟠 needs-a-spec. |
...
| [`backlog/gamify-the-public-sim.md`](backlog/gamify-the-public-sim.md) | **Not an audit item, and deliberately not a design.** Evidence for the fourth part of the owner's chat-first steer — *"Gamify this for regular people"* — which [`implementation-plan.md`](implementation-plan.md):1598 records as *"NOT yet specified and must not be guessed at"*. Who visits, what they loved, what disappoints, each claim carrying a URL and a date; then **five** priced candidates with a test plan and a counter-argument each | 🔵 **Research brief, 2026-09-05 — awaiting the owner's steer; it does not choose the game.** Two questions in its §0 unblock the build. **Its §1 is a precondition, not a candidate:** the Talk box measures `0 × 0` on load and `y = 2 095` after `CONTROLS`, so any gamification that does not first make the turn *reachable* is decoration — this brief refuses to rank around that. Ranked 🅐 reachable first turn (**S/M**) → 🅒 knock-knock turn-taking (**S**, recommended) → 🅓 thinking-out-loud (**S/M**) → 🅑 missions off the recovered `ONBOARD_MODULES` (**M**) → 🅔 on-ramp progress (**S/M**). **A new source this audit did not carry:** [`Noonster77/openmoxie`](https://github.com/Noonster77/openmoxie) — *"OpenMoxie Family Edition"*, [issue #63](https://github.com/jbeghtol/openmoxie/issues/63) (2026-08-21), a practitioner running a **physical** Moxie who independently built trivia, jokes with a turn-taking knock-knock pause, and facts-as-latency-cover; two candidates port those **behaviours** with credit, never code. Scan 2 of `community-signals.md` recorded both forks as having empty *issue trackers* — correct, and their **READMEs** are where the content was. **It also hardens a standing gap:** r/MoxieRobot is not merely unreachable but **structurally blocked to this agent** (`WebSearch` → `400 … not accessible to our user agent: ['reddit.com']`), so it is **owner-only or permanent** — and §4.1's upstream *"Animal Faces … missing"* line is another row for `community-signals.md` §3's *already-solved* table, since our face catalog shipped in PR #36/#47 |
| [`backlog/insights.md`](backlog/insights.md) | **BEYOND #5** (insights that mean something) — the event **vocabulary** the durable telemetry does not have, which parent questions it can and cannot answer, and how aggregation stays inside the `LoggingPolicy` contract | 🟢 **brief v1, 2026-09-04, 678 lines — UNLISTED HERE UNTIL 2026-09-06**, during which time §4.4 #8 ranked the item 🟠 *needs-a-spec*. §0 states the hardware ceiling first: no physical Moxie has ever sent this server a `Packet`, so every bucket boundary is chosen, not measured, and is an env var for that reason |
| [`backlog/mobile-first-visit.md`](backlog/mobile-first-visit.md) | **the owner's mobile steer** (a), (b) and (c) — the Talk box measured at **2 095 px below the fold** on a 390×844 phone: *not broken, buried* | 🟢 **SHIPPED 2026-09-05 on `feat/composer`** — `#chat-dock` as the page's own bottom row, the four controls **moved, not copied** (a second control is the trap this brief named); `sim/test_mobile_layout.mjs` went 66 → 222 checks, run against the pre-change page first for **81 failures of 206**. (d) *"gamify"* deliberately not built |
| [`backlog/vendor-the-readme-hero.md`](backlog/vendor-the-readme-hero.md) | the README hero was an off-site image the live docs page could not load | 🟢 **SHIPPED 2026-09-04** — vendored at `sim/web/img/sim-hero.png`; `img-src` untouched; `docs.js` resolves a doc's `<img src>` against that doc's repo path |
...
> 🩹 **2026-09-06 — counted again, and the rule caught itself a third time.** The folder holds **16**
> briefs (excluding `README.md`); this table held **13** rows. The three added above were
> [`insights.md`](backlog/insights.md), [`mobile-first-visit.md`](backlog/mobile-first-visit.md) and
> [`vendor-the-readme-hero.md`](backlog/vendor-the-readme-hero.md). **The first one cost something:**
> `insights.md` is a **678-line v1 brief dated 2026-09-04** whose own header says *"§3 removes that
> decision"* — the vocabulary decision §4.4 #8 was still calling a blocker two days later. An item was
> ranked 🟠 *needs-a-spec* while its spec sat unlisted in the same folder. **A brief that no index names
> is, operationally, a brief that does not exist.**
...
### What this table got wrong — three times in twelve hours, five more on 2026-09-05, and five more on 2026-09-06

This section is rewritten rather than edited because its *prose* — not just its statuses — has now been
measurably wrong **thirteen recorded times**: three between 2026-09-03 and 2026-09-04 (rows 1–3, each of
which cost an agent run), five found by the 2026-09-05 refresh (rows 4–8), and **five more found by the
2026-09-06 re-verification pass (rows 9–13)**. They are kept because the
...
the thirteen survived.

> ⚠️ **2026-09-06 — the failure moved upstream of this table, and that is the new finding.** The 2026-09-05
> refresh hardened §4.4 and left §3 frozen behind a *"do not brief from here"* warning. On 2026-09-06 the
> orchestrator was mis-briefed **twice**, and **both misleading cells were §3 rows** — not §4.4 rows.
> A frozen section with a warning at the top is not a control, because a reader lands on a *cell*, not on
> a section header. §3's verdict cells now carry their own inline corrections (see §3.0), and the ratio is
> the finding: **16 of the 21 §3 `ADOPT`/`BEYOND` verdict cells re-measured on 2026-09-06 were stale.**
...
| 13 | *"§4.4 #8 Insights — 🟠 **needs-a-spec** … that choice wants a spec"* | **The spec was written two days before that cell.** [`backlog/insights.md`](backlog/insights.md) is 678 lines, dated 2026-09-04, and its header says *"§3 removes that decision"*. It was **missing from §4.3's own briefs table**, whose stated rule is *"count the folder; do not trust the sentence"* — 16 briefs, 13 rows. An item stayed ranked as unspec'd because the index that would have shown otherwise was itself stale |
| 12 | **§4.2 #1, the behavior planner, still carried 🔵 *"open — spec'd"*** | **P1 shipped.** [`performance.py`](../../mqtt/moxie_sdk/performance.py) is 839 lines and [`markup.py`](../../mqtt/supervisor/markup.py):7 makes `planner` the **default**. §4.3, §4.4 #10 and the plan all recorded it; §4.2 — the table an agent scans for open work — did not. Two indexes of the same fact, only one updated |
| 11 | *"§4.4 #3 Turnstile — 🟡 in flight"* and *"§4.4 #1 — 🟢 build-ready"* | **Both shipped.** Turnstile: [`turnstile.js`](../../functions/api/_lib/turnstile.js) (511 lines) called from [`chat.js`](../../functions/api/chat.js):199. #1: all three ceilings on `caches.default` — [`limits.js`](../../functions/api/_lib/limits.js):1638 / :1811 / :1933. **#1 was this page's own ⭐ pick**, which is the second consecutive refresh whose headline recommendation was already merged |
| 10 | §3.3 content versioning said **`none`** | `source_version` is in **6 files** under `mqtt/` — [`module.py`](../../mqtt/moxie_sdk/content/module.py):113, [`packs.py`](../../mqtt/moxie_sdk/content/packs.py):150-157, and four more. **This cell mis-briefed the orchestrator on 2026-09-06.** §3.0 had recorded it closed since 2026-09-03 and the row was read anyway |
| 9 | §3.2 LLM response tags said *"nothing parses tags out of model text"* | **False since PR #6.** [`actions.py`](../../mqtt/moxie_sdk/actions.py):112 `parse_action_tags` + :103 `tidy_spoken_text`, reachable from `llm_app.py`:407/:442/:480, `content_app.py`:180/:414 and `webhook_app.py`:59. **This cell mis-briefed the orchestrator on 2026-09-06** — the near-miss that commissioned this pass |
...
taken from a commit subject. **The first two rows were added by the 2026-09-06 re-verification pass and
each closes a row that was still ranked open one section below.**

| Landed | What it actually does | Where it is proved |
|---|---|---|
| **2026-09-06 — the shared window and budget tier, completed** | **§4.4 #1 in full, and it was this page's ⭐ pick.** The per-IP **hour** and **day** windows share ONE cache entry (one round trip for two scales, which is how the latency objection was *answered* rather than overruled — each scale's bucket is stamped in the body, so a count from another bucket reads as zero), and the unit budget gained its **day** twin on its own ledger because hour and day roll on different clocks. Still per-colo, still fail-open, still **not** a global ceiling | [`functions/api/_lib/limits.js`](../../functions/api/_lib/limits.js):20-22 (the header claim) · `sharedWideWindow` :1638 · `sharedBudgetVerdict` :1811 · `sharedDayBudget` :1933 · the design note that records which objection each half answered, :1229-1244 |
| **2026-09-05 — Turnstile** | **§4.4 #3.** A bot without a real browser can no longer reach the one `fetch()` on `/api/chat`. Enforcement is config-gated, so previews and forks stay inert; a refusal refunds its 3 units and still releases the concurrency slot | [`functions/api/_lib/turnstile.js`](../../functions/api/_lib/turnstile.js) (511 lines) · [`chat.js`](../../functions/api/chat.js):74 (import), :168-199 (the position, argued in place) · [`sim/web/turnstile.js`](../../sim/web/turnstile.js) (browser half) · [`sim/test_turnstile.mjs`](../../sim/test_turnstile.mjs) (1 986 lines; §6 proves the slot release two ways) |
...
|--:|---|:--:|:--:|:--:|---|
| 1 | ~~**The hour/day windows and the unit budget on the Cache API tier**~~ | ① | no — **the public page** | 🟢 **SHIPPED IN FULL — re-verified against the code 2026-09-06; this row AND this page's ⭐ pick were stale** | 🩹 **All three halves are on the shared tier.** [`limits.js`](../../functions/api/_lib/limits.js):20-22 states it in the file header — *"SINCE 2026-09-06 EVERY PER-IP WINDOW AND EVERY UNIT-BUDGET CEILING IS ALSO KEPT IN THE CACHE API"* — and the functions are there to read: **`sharedWideWindow`** :1638 (the per-IP **hour and day** windows, built 2026-09-06), **`sharedBudgetVerdict`** :1811 (the **unit budget hour**, built 2026-09-05), **`sharedDayBudget`** :1933 (its **day** twin). The row's own scoping advice is now the *record of a decision that was answered rather than overruled*: :1229 keeps the latency objection standing and explains that both scales share **one** entry so they cost one round trip, not two, and :1237-1244 keeps the unit budget's prediction beside the one thing it got wrong (`slot.refundBudget()` did not exist when it was written, so *"the same fail-open rules apply verbatim"* was false of the refund path). **What is genuinely still per-isolate, and the header says so at :41-45 so this cannot drift again:** the **concurrency ceiling**, its FIFO, and `/api/health`'s `budget`/`load` probe — the first two refused a place deliberately, because a lost give-back leaks a slot for ever and fails **closed**. *Original text follows, kept because the reasoning was sound and only the inputs rotted:* **The loosest remaining *spend* ceiling on the one surface strangers can reach.** PR #146 moved the *minute* window into `caches.default`, shared per-COLO; the hour and day windows and the unit budget are still one isolate's `Map`. **Read [`limits.js`](../../functions/api/_lib/limits.js):853-863 before scoping this, because it already decided the order and the two halves are not equal:** the **unit budget** is *"deliberately next, not now … §4.6.1 orders it second"*, one more `match` + `put` on `.../units/<hour bucket>` under the same fail-open rules; the **hour and day windows** were *"rejected on the latency budget"*, so reviving them means re-arguing a measurement rather than writing code. So the buildable slice is **the unit budget**, and the honest second half is *"either revive the hour/day rejection with new numbers or retire it in writing"*. Net: the cheapest cap is global and the **expensive** one is not. Nothing is owed first: the reason this waited — assumption 13 — was retired on 2026-09-05 for exactly this design, because the Cache API needs no binding ([`live-sim-demo.md`](backlog/live-sim-demo.md):1364). It is also **#4 on the owner's own list**, and the highest item there that is neither shipped nor owner-blocked |
| 2 | **`'unsafe-inline'` in `style-src`** | ① | no — **the public page** | 🟠 **BLOCKED by a third party — measured 2026-09-05, not build-ready** | The last `'unsafe-inline'` on the origin the public reaches — [`_headers`](../../sim/web/_headers) — and the owner's ⑤. **Ranked honestly, not dramatically:** a style injection can neither load a script nor exfiltrate (`connect-src 'self'` has a *measured* kill on this site), so it is a much smaller hole than the script one #137 closed. **This row used to read 🟢 build-ready on the strength of `_headers`' own stated blocker, and BOTH were wrong.** `_headers` said `'unsafe-inline'` had to stay because the JS animates `el.style.transform` on a rAF loop *"which no hash can cover"* — the same error the file already made about `el.onclick` and scripts. Measured in Chrome under `style-src 'self'`: a rAF loop writing `el.style.transform`, `.opacity` and `setProperty` fires **zero** `securitypolicyviolation` events and every write applies, because a **CSSOM** write is not a `<style>` element, a `<link rel=stylesheet>` or a `style` **attribute**. The real blocker is `docs.html`'s diagram renderer: `vendor/mermaid.min.js` builds its theme at runtime, so `mermaid.render()` returns an SVG **string** carrying a `<style>` element and inline `style=` attributes that `docs.js` assigns via `innerHTML` — **23 refusals on one two-node diagram (2 `style-src-elem`, 21 `style-src-attr`), after which it rendered UNSTYLED**, and the docs ship **63 diagrams across 46 files**. No hash covers it (bytes differ per diagram) and `'unsafe-hashes'` does not help. Our own inline styles — 4 `<style>` blocks, 19 `style=` attributes, 9 more generated by `cloud.js`/`docs.js`, and the `<style>` `turnstile.js` injects — *are* deletable by #137's method, but that buys nothing while mermaid keeps the keyword in the directive. **Now guarded rather than described:** [`sim/test_csp.mjs`](../../sim/test_csp.mjs) block 10 derives the strict policy from the shipped line, proves the CSSOM finding and the negative control, and carries an **inverting** assertion that reddens the day mermaid stops needing this — at which point the fix is to delete the keyword |
| 3 | ~~**Turnstile**~~ | ① | no — **the public page** | 🟢 **SHIPPED 2026-09-05 — re-verified against the code 2026-09-06; "in flight" is stale** | 🩹 **Both halves exist and the enforcement point is deliberate.** Server: [`functions/api/_lib/turnstile.js`](../../functions/api/_lib/turnstile.js) (**511 lines**), imported at [`chat.js`](../../functions/api/chat.js):74 and called at :199 — **after** `admit()`, the input caps, the context check and the safety floor, and **immediately before the only `fetch()` in the file**, for three reasons written out in place at :168-197 (cheapest refusal first; `admit()` must protect siteverify, not the reverse; a locally-blocked utterance must not buy a verification). A refusal refunds the 3 units and still releases the concurrency slot from inside the `try`. Browser: [`sim/web/turnstile.js`](../../sim/web/turnstile.js) — one fresh token per send, per route, inert with no sitekey so forks and `file://` stay unbroken. Proof: [`sim/test_turnstile.mjs`](../../sim/test_turnstile.mjs) (**1 986 lines**), whose §6 proves the slot release two ways. **Unchanged and still true:** it removes the cheapest attack and does **not** bound the bill — which is why row #1 mattered separately. *Original cell follows:* 🟡 in flight 2026-09-05 — NEVER owner-blocked; this cell was wrong. A bot can still spend the gateway budget *inside* the per-IP caps; the caps bound the blast radius, they do not stop it. **Verified absent, not assumed:** `grep -rli turnstile functions/ sim/web/` matches exactly one line, [`limits.js`](../../functions/api/_lib/limits.js):382, and it is a comment saying Turnstile *is* P1. The `grep` finding was accurate. What followed it was not: the row claimed this **could not start without an owner action**, and that was never true. The widget already existed, and the Turnstile API returns an existing widget's `secret` on a plain `GET`, so an agent could read it and write it straight into the Pages production variables. Ranked high for value, yes — but **not** because an agent cannot do it |
...
| 8 | **Insights that mean something** (BEYOND #5's second half) | ①③ | no (a parent's console) | 🟢 **build-ready — 🩹 corrected 2026-09-06; the 🟠 was stale and the spec was two days old when it was written** | 🩹 [`backlog/insights.md`](backlog/insights.md) is a **678-line v1 brief dated 2026-09-04** and its header answers this cell directly: *"The audit states the blocker in its own words … **§3 removes that decision**"*, with §4 saying which parent questions the choice can and cannot answer, §5 the vocabulary itself and §6 how aggregating stays inside the `LoggingPolicy` contract. The brief was **missing from §4.3's own briefs table** until 2026-09-06, which is how a spec'd item stayed ranked as unspec'd. The **console half is further along than this cell implies too** — the 📈 card already renders a real zero-filled week ([`server/static/app.js`](../../server/static/app.js):255, `weekBars` :267, `refreshInsights` :330). *What is genuinely still missing:* sessions, activity mix, mood trend and time-of-day patterns — the five things BEYOND #5 names. *Original cell follows:* The durable half shipped (#55) and the *interesting* half did not: no sessions, no activity mix, no mood trend, no *"what did we talk about this week"*. 🟠 because the blocker is a **vocabulary**, not an implementation — `Packet.event_name` is a free string and our corpus recovers no module-scoped event — so the slice is a choice between deriving them from `mentor_behaviors` + conversation history and *emitting* our own named events. That choice wants a spec. **New constraint since #136:** whatever is derived must obey `telemetry_policy`, because `mentor_behaviors` is now gated by it |
...
> 🩹 **Corrected 2026-09-06 — this sentence is now wrong in the direction this page always rots.** Of the
> five it names, **#1 shipped in full** (see its row) and **#5 had already shipped** when the sentence was
> written. What is actually startable, re-measured against the code today: **#2** (`style-src` — still
> `'unsafe-inline'` at [`sim/web/_headers`](../../sim/web/_headers):247, and the blocker is a third party,
> so read the 2026-09-05 second-pass note at `_headers`:163-233 before scoping it), **#6** (`POST
> /content/try` — still deliberately absent, [`moxie_runtime.py`](../../mqtt/supervisor/moxie_runtime.py):4602)
> and **#7** (the printed launch-card sheet — still absent, [`launch_cards.py`](../../mqtt/moxie_sdk/launch_cards.py):139).
> **Three build-ready items out of twelve, not five.** The ordering is unchanged: it is the owner's, and
> nothing below has been re-ranked by this refresh — only re-measured.
...
### ⭐ The single most valuable next slice — ~~the spend ceilings on the Cache API tier~~ **SHIPPED**

> 🩹 **2026-09-06 — THIS SECTION'S PICK IS BUILT, AND THAT IS THE SECOND TIME IN TWO DAYS.** The section
> below already opens by recording that it *"previously named content authoring P0, which had already
> merged as PR #128"*. It has now done the same thing again: **the hour and day windows and the unit
> budget (hour *and* day) are all on the shared `caches.default` tier** — [`limits.js`](../../functions/api/_lib/limits.js):20-22,
> `sharedWideWindow` :1638, `sharedBudgetVerdict` :1811, `sharedDayBudget` :1933. The three reasons below
> are each still *correct as reasoning* and each now has a stale input, which is exactly the shape this
> page keeps producing.
>
> **What this does NOT mean.** It does not promote anything. The ordering below is the owner's steer —
> *"anything that makes a stranger's visit to `moxie.mattvalancy.com/sim` better, safer or cheaper
> outranks anything that serves a robot none of us has"* — and this refresh did not re-rank a single row;
> it only re-measured them. On the list as it already stands, the next public-page item that is build-ready
> is **#2 (`style-src`)**, which the runner-up paragraph at the end of this section already named and
> already explained the case for. It is promoted by #1 and #3 shipping, not by anyone's judgement.
>
> **What is still true and still unbuilt** in this area, so nobody reads "shipped" as "finished": the
> **concurrency ceiling** and its FIFO remain per-isolate on purpose, because a lost give-back leaks a
> slot for ever and fails **closed** ([`limits.js`](../../functions/api/_lib/limits.js):41-45); a burst
> still defeats the shared counters (31 concurrent increments stored 9, :26-27); and they are **per-colo,
> never global** — no sentence anywhere may call them a global ceiling (:51-55).
...
*The pick below is kept verbatim, and it is now history rather than a recommendation.* The pick was
**#1: the hour/day windows and the unit budget, moved onto the same `caches.default` tier the
