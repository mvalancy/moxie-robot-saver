...
| 6 | **Sandboxed content extensions** | Their `METHOD` globals and conversation `code` fields are `exec()` with a 10-second timeout — powerful, and un-shareable. A capability-scoped module runtime (declared permissions, no filesystem/network by default, resource limits) makes community content packs safe to install. | **L** | 🟡 **split — the security fix inside this row SHIPPED; the extension language did not.** 🟢 **Done:** the `SandboxedEnvironment` fix this row's survey turned up shipped alone as PR #56 (2026-09-02), gained its container dependency in PR #62, and was completed by PR #78 (2026-09-03) when the *jinja2-less fallback* renderer turned out to have the same hole by a different route (a bare dotted path walked with `getattr` reaches `os.environ`) — `sim/tests/test_render_sandbox.py` (14) · `test_render_sandbox_parity.py` · `test_content_pack_sandbox.py` (46). 🔵 **Open, and unchanged:** the capability-scoped extension runtime itself — nothing of §3-onward of the brief is built. Contract-level brief: [`backlog/sandboxed-extensions.md`](backlog/sandboxed-extensions.md). Our answer today is a refusal — `ContentApp` never `exec`s a module's `code`, so an imported pack cannot run anything (the *right* P0 posture, which content packs (ADOPT #5) keeps: `code` travels as inert data behind a ⚠️). The cost is that a pack carries only prompts and regexes — importing upstream's `MoxieTime`/`MoxieTimers` gives you a global that matches and then does nothing. **The brief's chosen answer:** extensions run **in our cloud, never on the robot**; a pack item gains an `extension` field holding a *declarative rule list over a total JSON-AST expression language* — no `exec`, no parser, no loops, no user functions, and no name that resolves to a host object, because the fact base handed to the evaluator is a pre-built plain-JSON dict. Every effect sits behind a declared capability whose set must **equal** what the AST uses (so the parent's plain-English grant list is provably complete); default-granted is only `{say, handled, session, child.nickname}`; the budget is carved **out of** `MOXIE_BRAIN_BUDGET_S`; a breach discards the effect list whole and falls through to today's path, so **Moxie keeps talking**. Justified by reading upstream's entire executable library at `c8c2d38` — 6 `code` strings / 9 hooks / 4 modules, **not one of which iterates** — all six hand-ported in the brief as the conformance golden set. WASM (P2 door) and an embedded VM are evaluated and rejected for P0: neither is reviewable, and only ours runs unchanged in a Cloudflare Worker *and* the Python supervisor. **A proven live finding came out of the survey:** `render.py` renders a pack-importable `prompt` through a plain `jinja2.Environment`, so template injection is server-side code execution wherever `jinja2` is installed (not the shipped container, which has none and uses the safe minimal renderer) — P0's first commit was to be the `SandboxedEnvironment` fix — **it shipped alone as PR #56 and is done** |
